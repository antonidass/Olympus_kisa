"""
Обогащает уже собранный CapCut-драфт «Дионис и Ариадна» переходами,
...
24 сцены Диониса и без CTA-аутро (финальный круг на сцене 024).
...
Запуск (CapCut должен быть полностью закрыт, включая трей):
    python enrich_dionysus.py
    python enrich_dionysus.py --dry-run   # только показать план
"""
...
DIONYSUS_DIR = DRAFTS / "Дионис и Ариадна"
DIONYSUS_FILE = DIONYSUS_DIR / "draft_content.json"
MIDAS_FILE = DRAFTS / "Мидас и золотое прикосновение" / "draft_content.json"


# ─────────────────────────────────────────────────────────────────────
# Карта sid сцены → число шотов в треке.
# Должна совпадать со scene_structure_dionysus.py:
#   24 сцены с озвучкой, 24 mp4-шота.
# ─────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────
# План переходов под контекст мифа Диониса и Ариадны:
# от предательства на Наксосе к внезапной дионисийской музыке, свадьбе
# и финальному созвездию.
# ─────────────────────────────────────────────────────────────────────
...
PLAN: List[Tuple[str, str, float, str]] = [
    ...
    ("004", "7609529907026119941", 1.00, "Полутоновая вспышка"),
    ("005", "6724845717472416269", 0.80, "叠化 (Dissolve)"),
    ("006", "6724845717472416269", 0.80, "叠化 (Dissolve)"),
    ("007", "7234817586234397186", 0.70, "Глитч-вспышка"),
    ("008", "7340177833508999681", 1.10, "Зум с тряской 2"),
    ("009", "6724845717472416269", 0.80, "叠化 (Dissolve)"),
    ("010", "7340177833508999681", 1.10, "Зум с тряской 2"),
    ("011", "7234817586234397186", 0.70, "Глитч-вспышка"),
    ("012", "7550260993348177213", 1.00, "Пастельные блики"),
    ("013", "7464433696658001213", 1.00, "Переход-зум"),
    ("014", "6724239584663704071", 0.90, "Свист"),
    ("015", "7550260993348177213", 1.00, "Пастельные блики"),
    ("016", "7609529907026119941", 1.00, "Полутоновая вспышка"),
    ("017", "7159450506648097281", 1.00, "Размытие (шар)"),
    ("018", "7550260993348177213", 1.00, "Пастельные блики"),
    ("019", "7340177833508999681", 1.20, "Зум с тряской 2"),
    ("020", "6724845717472416269", 0.80, "叠化 (Dissolve)"),
    ("021", "7159450506648097281", 1.00, "Размытие (шар)"),
...
WHOOSH_TRANSITION_EFFECT_IDS = {
    ...
    "7574908666210471221",  # Резкий зум
    "7262258307128103425",  # Зум с тряской
    "7464433696658001213",  # Переход-зум
...
# ─────────────────────────────────────────────────────────────────────
# Видео-эффекты: интро-halftone + финальный круг.
# Дионис без CTA-аутро, поэтому финальный эффект ставим на 024.
# ─────────────────────────────────────────────────────────────────────
...
HALFTONE_AUDIO_NAME = "sentence_002_v7.mp3"
...
def apply_video_effects(draft: dict, library: dict) -> List[str]:
    ...
    for sid, eff_id, label in EFFECT_PLAN:
        ...
        ve_mat["time_range"] = {
            "start": max(0, seg_dur - FINAL_EFFECT_DURATION_US) if sid == "024" else 0,
            "duration": min(seg_dur, FINAL_EFFECT_DURATION_US) if sid == "024" else seg_dur,
        }
...
# ─────────────────────────────────────────────────────────────────────
# Геометрия видео: нейтральная. Масштаб/смещение из CAPCUT.md применяются
# только вручную, если конкретный ролик этого требует.
# ─────────────────────────────────────────────────────────────────────
...
def apply_video_geometry(draft: dict) -> List[str]:
    ...
    for seg in main.get("segments", []):
        ...
        uniform["value"] = 1.0
    log.append(
        f"  video.main: scale=1.0, UI Y=0, "
        f"transform.y=0.0000000000 ({len(main.get('segments', []))} сегм.)"
    )
    return log
...
def main() -> int:
    ...
    if not DIONYSUS_FILE.is_file():
        print(f"Не нашёл драфт Диониса: {DIONYSUS_FILE}")
        print("Сначала запусти: python build_dionysus.py")
        return 1
    ...
    print(f"Читаю Диониса: {DIONYSUS_FILE}")
    draft = json.load(open(DIONYSUS_FILE, encoding="utf-8"))
    ...
    bkp = DIONYSUS_FILE.with_suffix(".json.enrich-backup")
    shutil.copy2(DIONYSUS_FILE, bkp)
    print(f"\nБэкап: {bkp.name}")

    json.dump(draft, open(DIONYSUS_FILE, "w", encoding="utf-8"),
              ensure_ascii=False, separators=(",", ":"))
    for tgt_name in ("template-2.tmp", "draft_content.json.bak"):
        tgt = DIONYSUS_DIR / tgt_name
        try:
            shutil.copy2(DIONYSUS_FILE, tgt)
