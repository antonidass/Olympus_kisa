"""
Лёгкий зум и выравнивание по верхнему краю всех видео-сегментов главной
дорожки «Сизифов труд».

Канон CapCut:
  • scale = 1.04
  • поле «Положение → Y» в UI CapCut = -77
  • в draft_content.json это хранится как transform.y = -77 / 1920
...
Запуск (CapCut должен быть закрыт):
    ...
    python pyCapCut/zoom_videos_sisyphus.py --scale 1.04 --ui-y -77
...
def main() -> int:
    ...
    p.add_argument("--ui-y", type=float, default=DEFAULT_UI_Y, help="Значение поля Y в UI CapCut.")
    p.add_argument("--reset", action="store_true", help="Сбросить scale до 1.0.")
    args = p.parse_args()

    scale = 1.0 if args.reset else args.scale
    transform_y = 0.0 if args.reset else args.ui_y / FRAME_HEIGHT
    ...
    print(f"\n✓ Поставил scale={scale}, UI Y={0 if args.reset else args.ui_y} на {n} видео-сегментов главной дорожки.")
    print(f"  draft_content.json transform.y={transform_y:.10f}")
    if scale > 1.0:
