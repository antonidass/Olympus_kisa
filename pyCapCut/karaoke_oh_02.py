"""
Караоке-субтитры для «От Хаоса до Олимпа — Ч.02 Власть Кроноса».

Адаптация karaoke_oh_01.py: пути контента ведут в
`content/От Хаоса до Олимпа/часть_02_Власть_Кроноса/` (сериал — поэтому
иерархия глубже, чем у одиночных мифов). Имя CapCut-драфта плоское:
«От Хаоса до Олимпа Ч.02 Власть Кроноса». Интро-карточка переписана
под «ОТ ХАОСА ДО ОЛИМПА / ЧАСТЬ 2». Остальной пайплайн идентичен ч.01.
...
Что делает:
  ...
  6. Кэширует транскрипции в pyCapCut/_karaoke_cache_oh_02.json.

Требования:
  - CapCut должен быть закрыт.
  - Запускать из CosyVoice venv (там whisper + CUDA):
    external\\CosyVoice\\.venv_cosyvoice\\Scripts\\python.exe pyCapCut\\karaoke_oh_02.py
...
# Имя CapCut-драфта (плоское, как в %LOCALAPPDATA%\CapCut\...\com.lveditor.draft\<имя>)
PROJECT_NAME = "От Хаоса до Олимпа Ч.02 Власть Кроноса"
# Путь к контенту в репо (сериал — поэтому иерархия глубже, чем у одиночных мифов)
CONTENT_DIR = PROJECT_ROOT / "content" / "От Хаоса до Олимпа" / "часть_02_Власть_Кроноса"
AUDIO_DIR = CONTENT_DIR / "voiceover" / "audio" / "approved_sentences"
REF_TEXTS_DIR = CONTENT_DIR / "voiceover" / "texts"
CACHE_FILE = Path(__file__).resolve().parent / "_karaoke_cache_oh_02.json"
...
# Строка 1: "ДИОНИС И АРИАДНА" (белый), строка 2: "МИФ ЗА МИНУТУ" (бордовый #cb3a3a).
INTRO_FONT_SIZE = 17.0                  # эталон канала (см. CAPCUT.md §5.1 + memory feedback_intro_font_size)
...
def transcribe_all(sentences: list[dict], model_name: str, allow_whisper: bool) -> None:
    ...
    try:
        ...
        raise SystemExit(
            ...
            r"  external\CosyVoice\.venv_cosyvoice\Scripts\python.exe pyCapCut\karaoke_oh_02.py"
        ) from e
...
def build_intro_steps(intro_sentence: dict) -> list[tuple[int, int, str]]:
    """
    Один двухстрочный интро-титр на всю длительность sentence_002:
...
def build_intro_content_json(text: str, size: int,
    ...
    if nl_pos < 0:
        ...
        styles = [
            style([0, nl_pos], white),                  # "ДИОНИС И АРИАДНА"
...
def main() -> int:
    ...
    # --- Диапазон интро (scene_001 содержит два аудио: hook + sentence_002 intro)
    intro_sentence = next(
        (s for s in sentences if re.match(r"sentence_002", s["fname"])),
        None,
    )
    if intro_sentence:
    ...
    print("Открой CapCut -> проект «От Хаоса до Олимпа Ч.02 Власть Кроноса» -> проверь.")
    return 0
