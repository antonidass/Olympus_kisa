"""
Сборка CapCut-драфта «Персей и Медуза» через pyCapCut.

Скопировано с build_orpheus.py с подменой путей и структуры сцен:
23 предложения озвучки → 24 видеошота в 22 таймлайн-сценах. CTA-аутро
у мифа нет — финальный кадр всегда заканчивается ровно по последней озвучке.

Использование:
    python build_perseus.py                                  # автоопределение папки CapCut
    python build_perseus.py --drafts "D:\\...\\com.lveditor.draft"
    python build_perseus.py --name "Персей v2"
    python build_perseus.py --dry-run                        # только напечатать план таймлайна
"""
...
from scene_structure_perseus import SCENES, Scene
...
DEFAULT_PROJECT_NAME = "Персей и Медуза"
...
MYTH_ROOT = PROJECT_ROOT / "content" / "Персей и Медуза"
...
STICKER_PLAN = [
    # visual scene file, sticker image, x, y, scale, sfx
    ("scene_04_v1.mp4", "scene_04_wedding_registry_trap_scroll.jpg", -0.54, 0.39, 0.245, "pac"),
    ("scene_05_v1.mp4", "scene_05_warrior_hero_loot_card.jpg", 0.55, 0.40, 0.245, "coin"),
    ("scene_06_v1.mp4", "scene_06_bear_trap_snapped_shut.jpg", -0.53, 0.36, 0.245, "mouse"),
    ("scene_11_v1.mp4", "scene_11_shared_eye_broadcast_icon.jpg", 0.55, 0.38, 0.245, "pac"),
    ("scene_14_v1.mp4", "scene_14_stealth_mode_hud_badge.jpg", -0.55, 0.39, 0.245, "mouse"),
    ("scene_20_v1.mp4", "scene_20_mystery_wedding_gift_box.jpg", 0.54, 0.39, 0.245, "pac"),
    ("scene_21_v1.mp4", "scene_21_trophy_statue_achievement_medal.jpg", -0.54, 0.38, 0.245, "coin"),
]
...
def build_draft(drafts_folder: Path, project_name: str) -> Path:
    try:
        ...
        from pycapcut import trange, TransitionType, TextStyle, ClipSettings, KeyframeProperty  # type: ignore
    except ImportError as e:
        raise SystemExit(
    ...
    for shot_file, sticker_file, x, y, scale, sfx_key in STICKER_PLAN:
        ...
        bounce_mid = min(300_000, dur_us // 3)
        bounce_end = min(600_000, max(bounce_mid + 1, dur_us // 2))
        sticker_seg.add_keyframe(KeyframeProperty.position_y, 0, y)
        sticker_seg.add_keyframe(KeyframeProperty.position_y, bounce_mid, y + 0.07)
        sticker_seg.add_keyframe(KeyframeProperty.position_y, bounce_end, y)
    ...
    # ── Субтитры ──
    # Только интро-сегмент сцены 002 как шаблон стиля для karaoke-скрипта.
    # Остальные SCENE_TEXTS не добавляем — их перекроет karaoke_perseus.py.
    intro_ts = next((ts for ts in plan if ts.scene.sid == "002"), None)
...
def main() -> int:
    p = argparse.ArgumentParser(description="Собрать CapCut-драфт «Персей и Медуза» через pyCapCut.")
    ...
    if drafts is None or not drafts.is_dir():
        print(
            ...
            "  python build_perseus.py --drafts "
