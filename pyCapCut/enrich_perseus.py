"""
Обогащает уже собранный CapCut-драфт «Персей и Медуза» переходами,
...
22 сцены Персея и без CTA-аутро (финальный круг на сцене 022).
...
PERSEUS_DIR = DRAFTS / "Персей и Медуза"
PERSEUS_FILE = PERSEUS_DIR / "draft_content.json"
MIDAS_FILE = DRAFTS / "Мидас и золотое прикосновение" / "draft_content.json"


# ─────────────────────────────────────────────────────────────────────
# Карта sid сцены → число шотов в треке.
# Должна совпадать со scene_structure_perseus.py:
#   22 сцены с озвучкой, 24 mp4-шота (2-шотные: 004/006/013/020).
# ─────────────────────────────────────────────────────────────────────

SCENE_LAYOUT: List[Tuple[str, int]] = [
    ("001", 1), ("002", 1), ("003", 1), ("004", 1), ("005", 1),
    ("006", 1), ("007", 1), ("008", 1), ("009", 1), ("010", 1),
    ("011", 1), ("012", 1), ("013", 1), ("014", 2), ("015", 1),
    ("016", 1), ("017", 1), ("018", 2), ("019", 1), ("020", 1),
    ("021", 1), ("022", 1),
]
# Сумма: 24
...
# ─────────────────────────────────────────────────────────────────────
# План переходов под контекст мифа Персея:
...
PLAN: List[Tuple[str, str, float, str]] = [
    ...
    ("005", "7340177833508999681", 1.10, "Зум с тряской 2"),
    ("006", "7234817586234397186", 0.70, "Глитч-вспышка"),
    ("007", "7550260993348177213", 1.00, "Пастельные блики"),
    ("008", "6724845717472416269", 0.80, "叠化 (Dissolve)"),
    ("009", "6724239584663704071", 0.90, "Свист"),
    ("010", "7550260993348177213", 1.00, "Пастельные блики"),
    ...
    ("015", "7609529907026119941", 1.20, "Полутоновая вспышка"),
    ...
    ("020", "7550260993348177213", 1.00, "Пастельные блики"),
    ("021", "7620344224734629138", 1.20, "Растяжение влево"),
...
# Персей без CTA-аутро, поэтому финальный эффект ставим на 022.
# ─────────────────────────────────────────────────────────────────────

EFFECT_PLAN: List[Tuple[str, str, str]] = [
    ("022", "7613711779025358087", "Финальный круг"),
]
...
HALFTONE_AUDIO_NAME = "sentence_003_v1.mp3"
...
def main() -> int:
    ...
    if not PERSEUS_FILE.is_file():
        print(f"Не нашёл драфт Персея: {PERSEUS_FILE}")
    ...
    print(f"Читаю Персея: {PERSEUS_FILE}")
    draft = json.load(open(PERSEUS_FILE, encoding="utf-8"))
    ...
    bkp = PERSEUS_FILE.with_suffix(".json.enrich-backup")
    shutil.copy2(PERSEUS_FILE, bkp)
    print(f"\nБэкап: {bkp.name}")

    json.dump(draft, open(PERSEUS_FILE, "w", encoding="utf-8"),
              ensure_ascii=False, separators=(",", ":"))
    for tgt_name in ("template-2.tmp", "draft_content.json.bak"):
        tgt = PERSEUS_DIR / tgt_name
        try:
            shutil.copy2(PERSEUS_FILE, tgt)
