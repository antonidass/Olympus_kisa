"""
Караоке-субтитры для «Каллисто и Аркас».
...
Что делает:
  ...
  4. Удаляет старые сценные субтитры, кроме интро-оверлея
     «КАЛЛИСТО И АРКАС / МИФ ЗА МИНУТУ».
  5. Добавляет новые текстовые сегменты — по одному на слово, КАПСОМ.
  6. Кэширует транскрипции в pyCapCut/_karaoke_cache_kallisto.json.

Требования:
  - CapCut должен быть закрыт.
  - Запускать из CosyVoice venv (там whisper + CUDA):
        external\\CosyVoice\\.venv_cosyvoice\\Scripts\\python.exe pyCapCut\\karaoke_kallisto.py
...
CACHE_FILE = Path(__file__).resolve().parent / "_karaoke_cache_kallisto.json"
...
def transcribe_all(sentences: list[dict], model_name: str, allow_whisper: bool) -> None:
    ...
    try:
        ...
        raise SystemExit(
            ...
            r"  external\CosyVoice\.venv_cosyvoice\Scripts\python.exe pyCapCut\karaoke_kallisto.py"
        ) from e
...
def build_intro_content_json(text: str, size: int,
    ...
    if nl_pos < 0:
        ...
        styles = [
            style([0, nl_pos], white),                  # "КАЛЛИСТО И АРКАС"
...
def main() -> int:
    ...
    print("Открой CapCut → проект «Каллисто и Аркас» → проверь.")
    return 0
