"""
Караоке-субтитры для «Аполлон и Кассандра».
...
Что делает:
  ...
  6. Кэширует транскрипции в pyCapCut/_karaoke_cache_cassandra.json.

Требования:
  - CapCut должен быть закрыт.
  - Запускать из CosyVoice venv (там whisper + CUDA):
    external\\CosyVoice\\.venv_cosyvoice\\Scripts\\python.exe pyCapCut\\karaoke_cassandra.py
...
PROJECT_NAME = "Аполлон и Кассандра"
...
CACHE_FILE = Path(__file__).resolve().parent / "_karaoke_cache_cassandra.json"
...
# Интро: один двухстрочный титр.
# Строка 1: "АПОЛЛО́Н И КАССА́НДРА" (белый), строка 2: "МИФ ЗА МИНУТУ" (бордовый #cb3a3a).
# Канон размера: 16.0 — на длинных названиях 17 всё ещё доминировал в кадре,
# поэтому окончательно 16 (см. feedback_intro_font_size).
...
def transcribe_all(sentences: list[dict], model_name: str, allow_whisper: bool) -> None:
    ...
    try:
        ...
        raise SystemExit(
            ...
            r"  external\CosyVoice\.venv_cosyvoice\Scripts\python.exe pyCapCut\karaoke_cassandra.py"
        ) from e
...
def build_intro_content_json(text: str, size: int,
    ...
    if nl_pos < 0:
        ...
        styles = [
            style([0, nl_pos], white),                  # "АПОЛЛОН И КАССАНДРА"
