"""
Обогащает уже собранный CapCut-драфт «Цирцея и Одиссей» переходами,
видео-эффектами, фейдом музыки, громкостями и whoosh-SFX.

Копия enrich_femida.py с подменой SCENE_LAYOUT под 19 сцен и плана
переходов под арку мифа Цирцеи. EFFECT_PLAN пуст («Финальный круг»
запрещён каноном).

Тянем готовые transition / video_effect шаблоны из живого драфта Мидаса.

Запуск (CapCut должен быть полностью закрыт):
    python enrich_circe.py
    python enrich_circe.py --dry-run
"""
...
# ─────────────────────────────────────────────────────────────────────
# Карта sid сцены → число шотов в треке: 19 сцен с одним шотом каждая.
# ─────────────────────────────────────────────────────────────────────
...
SCENE_LAYOUT: List[Tuple[str, int]] = [
    ...
    ("016", 1), ("017", 1), ("018", 1), ("019", 1),
]


# ─────────────────────────────────────────────────────────────────────
# План переходов под арку мифа «Цирцея и Одиссей».
# Драматургия: Dissolve как база, Свист на причаливании, Глитч-вспышка
# на моментах трансформации, Переход-зум на действиях, Размытие-шар
# на пиршестве, Растяжение влево на прощании. Запрещённые переходы
...
PLAN: List[Tuple[str, str, float, str]] = [
    ...
    ("003", "6724239584663704071", 0.90, "Свист"),
...
HALFTONE_AUDIO_NAME = "sentence_002_v6.mp3"
...
def make_global_video_effect(effect_id: str, name: str) -> dict:
    return {
        ...
        "platform": "all",
        "render_index": 11000,
        "request_id": "20260524HALFTONECIRCE",
...
def apply_transitions(draft: dict, library: dict) -> List[str]:
    ...
    for plan_idx, (sid, eff_id, want_dur_s, label) in enumerate(PLAN):
        ...
        if template is None:
            log.append(f"  ⚠ effect_id {eff_id} ({label}) не нашёлся в Мидасе — пропуск")
            continue
...
def apply_video_effects(draft: dict, library: dict) -> List[str]:
    ...
    for sid, eff_id, label in EFFECT_PLAN:
        ...
        ve_mat["time_range"] = {"start": 0, "duration": seg_dur}
...
def main() -> int:
    ...
    if not CIRCE_FILE.is_file():
        print(f"Не нашёл драфт Цирцеи: {CIRCE_FILE}")
        print("Сначала запусти: python build_circe.py")
        return 1
    ...
    print(f"Читаю шаблоны Мидаса: {MIDAS_FILE}")
    ...
    bkp = CIRCE_FILE.with_suffix(".json.enrich-backup")
    ...
    for tgt_name in ("template-2.tmp", "draft_content.json.bak"):
        tgt = CIRCE_DIR / tgt_name
        try:
            shutil.copy2(CIRCE_FILE, tgt)
