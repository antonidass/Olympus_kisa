"""
Караоке-субтитры для «Цирцея и Одиссей».
...
Что делает:
  ...
  6. Кэширует транскрипции в pyCapCut/_karaoke_cache_circe.json.

Требования:
  - CapCut должен быть закрыт.
  - Запускать из CosyVoice venv (там whisper + CUDA):
    external\\CosyVoice\\.venv_cosyvoice\\Scripts\\python.exe pyCapCut\\karaoke_circe.py
...
CACHE_FILE = Path(__file__).resolve().parent / "_karaoke_cache_circe.json"
...
# Строка 1: "ЦИРЦЕ́Я И ОДИССЕ́Й" (белый), строка 2: "МИФ ЗА МИНУТУ" (бордовый #cb3a3a).
...
def transcribe_all(sentences: list[dict], model_name: str, allow_whisper: bool) -> None:
    ...
    try:
        ...
        raise SystemExit(
            ...
            r"  external\CosyVoice\.venv_cosyvoice\Scripts\python.exe pyCapCut\karaoke_circe.py"
        ) from e
...
def build_intro_content_json(text: str, size: int,
    ...
    if nl_pos < 0:
        ...
        styles = [
            style([0, nl_pos], white),                  # "ЦИРЦЕЯ И ОДИССЕЙ"
