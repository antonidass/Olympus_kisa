"""
Сборка CapCut-драфта «Каллисто и Аркас» через pyCapCut.

23 предложения озвучки → 24 видеошота в 23 таймлайн-сценах. Сцена 001
содержит ДВА аудио (хук sent_001 + интро sent_002) поверх одного кадра-
крючка (scene_01_v2.mp4). CTA-аутро нет — финальный кадр (sent_023 →
scene_24) заканчивается с trailing_pad=1.5.

Стикеры + SFX «приклеиваются» в сценах 02-19 (18 штук) — pixel-art
мем-каркасы с альфой из images/stickers/cutout/. Анимация качки
(bounce y → y+0.07 → y за 0.6с) добавляется отдельным скриптом
add_kallisto_sticker_bounce.py ПОСЛЕ build (pycapcut не умеет writes
keyframes напрямую — патчим draft_content.json).

Музыки нет — папка content/Каллисто и Аркас/music/ пустая; MUSIC_FILE
указывает на несуществующий файл → код пропускает music-трек. Когда
найдёшь фоновый трек — положи в music/kallisto_bg.mp3 либо подмени
MUSIC_FILE ниже.

Использование:
    python build_kallisto.py                                  # автоопределение папки CapCut
    python build_kallisto.py --drafts "D:\\...\\com.lveditor.draft"
    python build_kallisto.py --name "Каллисто v2"
    python build_kallisto.py --dry-run                        # только напечатать план таймлайна
"""
...
from scene_structure_kallisto import SCENES, Scene
...
MYTH_ROOT = PROJECT_ROOT / "content" / "Каллисто и Аркас"

SCENES_DIR = MYTH_ROOT / "video"
AUDIO_DIR = MYTH_ROOT / "voiceover" / "audio" / "approved_sentences"
STICKERS_DIR = MYTH_ROOT / "images" / "stickers" / "cutout"   # PNG с альфой

# Общие SFX/музыка для всех мифов — в корневом assets/.
ASSETS_DIR = PROJECT_ROOT / "assets"
# Музыки для «Каллисто» пока нет — путь специально указывает на
# несуществующий файл в локальной music/, чтобы код естественно
# пропускал music-трек. Позже положишь сюда трек и пересоберёшь.
MUSIC_FILE = MYTH_ROOT / "music" / "kallisto_bg.mp3"
...
# В STICKER_PLAN указывается имя соответствующего mp4-шота в треке main —
# стикер «прицепляется» к этому шоту, выравнивается через ClipSettings
# (x, y в нормализованных −1…+1, scale 0…1). Позиции чередуются ±0.54
# (правый/левый край) с y≈0.39 — стандартный pixel-art-карман канала.
STICKER_PLAN = [
    # shot file,           sticker png в cutout/,                                 x,     y,    scale, sfx
    ("scene_02_v1.mp4",  "scene_02_hunter_trophy_gold_achievement.png",      0.54,  0.39, 0.245, "coin"),
    ("scene_03_v2.mp4",  "scene_03_chastity_vow_scroll_seal.png",           -0.54,  0.40, 0.245, "pac"),
    ("scene_04_v1.mp4",  "scene_04_hud_target_reticle_lock.png",             0.54,  0.39, 0.245, "mouse"),
    ("scene_05_v1.mp4",  "scene_05_forbidden_error_window_column.png",      -0.54,  0.39, 0.245, "coin"),
    ("scene_06_v2.mp4",  "scene_06_disguise_loading_progress_bar.png",       0.54,  0.40, 0.245, "mouse"),
    ("scene_07_v1.mp4",  "scene_07_captcha_real_challenge_box.png",         -0.54,  0.39, 0.245, "pac"),
    ("scene_08_v2.mp4",  "scene_08_plot_twist_comic_burst.png",              0.54,  0.39, 0.245, "coin"),
    ("scene_09_v1.mp4",  "scene_09_positive_pregnancy_test_stick.png",      -0.54,  0.39, 0.245, "mouse"),
    ("scene_10_v2.mp4",  "scene_10_rejected_stamp_reputation_drop.png",      0.54,  0.39, 0.245, "pac"),
    ("scene_11_v1.mp4",  "scene_11_baby_notification_badge_alert.png",      -0.54,  0.40, 0.245, "coin"),
    ("scene_12_v1.mp4",  "scene_12_pokemon_evolution_transformation_flash.png", 0.54, 0.39, 0.245, "mouse"),
    ("scene_13_v1.mp4",  "scene_13_still_me_thought_bubble.png",            -0.54,  0.39, 0.245, "pac"),
    ("scene_14_v1.mp4",  "scene_14_kid_unlocked_rpg_shepherd.png",           0.54,  0.39, 0.245, "coin"),
    ("scene_15_v2.mp4",  "scene_15_level_up_fifteen_sparks.png",            -0.54,  0.40, 0.245, "mouse"),
    ("scene_16_v1.mp4",  "scene_16_wild_bear_appeared_encounter.png",        0.54,  0.39, 0.245, "pac"),
    ("scene_17_v1.mp4",  "scene_17_hug_incoming_bear_silhouette.png",       -0.54,  0.39, 0.245, "coin"),
    ("scene_18_v1.mp4",  "scene_18_enemy_detected_hunter_hud.png",           0.54,  0.39, 0.245, "mouse"),
    ("scene_19_v1.mp4",  "scene_19_press_x_attack_prompt.png",              -0.54,  0.40, 0.245, "pac"),
]


# ─────────────────────────────────────────────────────────────────────
# Длительность mp3 через mutagen (точный счётчик MPEG-фреймов).
# См. CAPCUT.md §7.1-7.2 — без квантования к границе кадра CapCut
# включает таймстретч и получается робовойс. floor_to_frame_us
# страхует от этого.
# ─────────────────────────────────────────────────────────────────────
...
def build_draft(drafts_folder: Path, project_name: str) -> Path:
    ...
    # Стикер живёт ВСЮ длительность сцены (правило обновлено 2026-05-21 на
    # Каллисто — было 1.5с фиксированно, теперь растягивается). SFX стикера
    # звучит в момент его появления (на старте сцены). Анимация качки
    # добавляется отдельным скриптом add_kallisto_sticker_bounce.py ТОЛЬКО
    # после ручных правок пользователя (см. CAPCUT.md §4.3 — три прохода).
    #
    # КРИТИЧНО: source_timerange.duration ОБЯЗАН совпадать с
    # target_timerange.duration для статичных PNG-стикеров. Иначе CapCut
    # анимирует "рамку" сегмента на target-длительности, а PNG показывается
    # только source — после source-длительности картинка "замирает", но
    # KFTypePositionY-keyframes продолжают двигать сегмент. Визуально это
    # выглядит как "трясущаяся рамка вокруг неподвижного стикера".
    # Передаём source_timerange явно через trange, чтобы он не пришёл из
    # material.duration (= 10800с заглушка для PNG).
    for shot_file, sticker_file, x, y, scale, sfx_key in STICKER_PLAN:
        ...
        sticker_seg = cc.VideoSegment(
            ...
            source_timerange=trange(0, dur_us),   # обязательно: src.dur = target.dur
            volume=0.0,
    ...
    if MUSIC_FILE.is_file():
        ...
        print(f"  WARN не нашёл {MUSIC_FILE}, музыку пропускаю (положи трек в music/ и пересобери)")
    ...
    # ── Субтитры ──
    # Только интро-сегмент (sent_002) как шаблон стиля для karaoke_*.py.
    # Остальные SCENE_TEXTS не добавляем — их перекроет karaoke_kallisto.py.
    # Сцена 001 содержит ДВА аудио: хук sent_001 + интро sent_002. Титул
    # «Каллисто и Аркас. Миф за минуту.» лежит на втором аудио (sent_002).
...
def main() -> int:
    p = argparse.ArgumentParser(description="Собрать CapCut-драфт «Каллисто и Аркас» через pyCapCut.")
    ...
    if drafts is None or not drafts.is_dir():
        print(
            ...
            "  python build_kallisto.py --drafts "
    ...
    print("  Дальше: python pyCapCut/add_kallisto_sticker_bounce.py  # добавит анимацию качки стикеров")
    print("  Потом:  открой CapCut -> Drafts -> «Каллисто и Аркас» -> правь / экспортируй.")
    return 0
