"""
Обогащает уже собранный CapCut-драфт «Фемида» переходами,
видео-эффектами, фейдом музыки, громкостями и whoosh-SFX. Копия
enrich_dionysus.py с подменой карты SCENE_LAYOUT, PLAN под 18 сцен
Фемиды. EFFECT_PLAN пуст (без CTA-аутро и финального круга — см.
правило feedback_banned_transitions_and_effects).
...
Запуск (CapCut должен быть полностью закрыт, включая трей):
    python enrich_femida.py
    python enrich_femida.py --dry-run   # только показать план
"""
...
FEMIDA_DIR = DRAFTS / "Фемида"
FEMIDA_FILE = FEMIDA_DIR / "draft_content.json"
MIDAS_FILE = DRAFTS / "Мидас и золотое прикосновение" / "draft_content.json"


# ─────────────────────────────────────────────────────────────────────
# Карта sid сцены → число шотов в треке.
# Должна совпадать со scene_structure_femida.py:
#   18 сцен с озвучкой, 18 mp4-шотов (по одному шоту на сцену).
# ─────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────
# План переходов под арку мифа Фемиды:
#   gravity-разрешённые переходы (Dissolve / Свист / Переход-зум /
#   Глитч-вспышка / Размытие шар / Бумажный шар / Растяжение).
#   ЗАПРЕЩЕНЫ (feedback_banned_transitions_and_effects):
#     7609529907026119941  Полутоновая вспышка
#     7550260993348177213  Пастельные блики
...
PLAN: List[Tuple[str, str, float, str]] = [
    ...
    ("003", "7234817586234397186", 0.70, "Глитч-вспышка"),
    ...
    ("008", "7234817586234397186", 0.70, "Глитч-вспышка"),
    ("009", "6724845717472416269", 0.80, "叠化 (Dissolve)"),
    ("010", "6724239584663704071", 0.90, "Свист"),
    # Бумажный шар (7249296835204878850) недоступен в Мидас-библиотеке — используем Dissolve как fallback
    ...
    ("014", "7159450506648097281", 1.00, "Размытие (шар)"),
    ("015", "7620344224734629138", 1.20, "Растяжение влево"),
    ("016", "7159450506648097281", 1.00, "Размытие (шар)"),
    ("017", "6724845717472416269", 0.80, "叠化 (Dissolve)"),
    # 018 — последняя сцена, переход не нужен
]
...
# (feedback_banned_transitions_and_effects).
# ─────────────────────────────────────────────────────────────────────
...
HALFTONE_AUDIO_NAME = "sentence_002_v10.mp3"
...
def main() -> int:
    ...
    if not FEMIDA_FILE.is_file():
        print(f"Не нашёл драфт Фемиды: {FEMIDA_FILE}")
        print("Сначала запусти: python build_femida.py")
        return 1
    ...
    print(f"Читаю Фемиду: {FEMIDA_FILE}")
    draft = json.load(open(FEMIDA_FILE, encoding="utf-8"))
    ...
    bkp = FEMIDA_FILE.with_suffix(".json.enrich-backup")
    shutil.copy2(FEMIDA_FILE, bkp)
    print(f"\nБэкап: {bkp.name}")

    json.dump(draft, open(FEMIDA_FILE, "w", encoding="utf-8"),
              ensure_ascii=False, separators=(",", ":"))
    for tgt_name in ("template-2.tmp", "draft_content.json.bak"):
        tgt = FEMIDA_DIR / tgt_name
        try:
            shutil.copy2(FEMIDA_FILE, tgt)
