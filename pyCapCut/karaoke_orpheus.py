"""
Караоке-субтитры для «Орфей и Эвридика».
...
Что делает:
  ...
  4. Удаляет старые сценные субтитры, кроме интро-оверлея
     «ОРФЕЙ И ЭВРИДИКА / МИФ ЗА МИНУТУ».
  5. Добавляет новые текстовые сегменты — по одному на слово, КАПСОМ.
  6. Кэширует транскрипции в pyCapCut/_karaoke_cache_orpheus.json.

Требования:
  - CapCut должен быть закрыт.
  - Запускать из CosyVoice venv (там whisper + CUDA):
        external\\CosyVoice\\.venv_cosyvoice\\Scripts\\python.exe pyCapCut\\karaoke_orpheus.py
...
THESEUS_NAME = "Орфей и Эвридика"
AUDIO_DIR = PROJECT_ROOT / "content" / THESEUS_NAME / "voiceover" / "audio" / "approved_sentences"
REF_TEXTS_DIR = PROJECT_ROOT / "content" / THESEUS_NAME / "voiceover" / "texts"
CACHE_FILE = Path(__file__).resolve().parent / "_karaoke_cache_orpheus.json"
...
DRAFT_DIR = LOCALAPPDATA / "CapCut" / "User Data" / "Projects" / "com.lveditor.draft" / THESEUS_NAME
...
# Строка 1: "ОРФЕЙ И ЭВРИДИКА" (белый), строка 2: "МИФ ЗА МИНУТУ" (бордовый #cb3a3a).
...
def transcribe_all(sentences: list[dict], model_name: str, allow_whisper: bool) -> None:
    ...
    try:
        ...
        raise SystemExit(
            ...
            r"  external\CosyVoice\.venv_cosyvoice\Scripts\python.exe pyCapCut\karaoke_orpheus.py"
        ) from e
...
def build_intro_steps(intro_sentence: dict) -> list[tuple[int, int, str]]:
    ...
    ts = [base_us + int(w["start"] * 1_000_000) for w in words[:6]]
    # гарантируем монотонность (иногда whisper даёт next.start < prev.end)
    ts = [max(ts[0], ts[i] if i == 0 else max(ts[i], ts[i - 1] + 1000)) for i in range(6)]

    u = [w.upper() for w in ws]
    text = f"{u[0]} {u[1]} {u[2]}\n{u[3]} {u[4]} {u[5]}"
    return [(ts[0], end_us - ts[0], text)] if end_us > ts[0] else []
...
def build_intro_content_json(text: str, size: int,
    ...
    if nl_pos < 0:
        ...
        styles = [
            style([0, nl_pos], white),                  # "ОРФЕЙ И ЭВРИДИКА"
...
def main() -> int:
    ...
    # "intro" мульти-стилем, а сценные тексты перекрывает пословное караоке.
    ...
    # --- Диапазон интро (второе предложение, sentence_002_*.mp3)
    intro_sentence = next(
    ...
    print("Открой CapCut → проект «Орфей и Эвридика» → проверь.")
    return 0
