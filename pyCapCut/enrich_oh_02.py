"""
Обогащает уже собранный CapCut-драфт «От Хаоса до Олимпа Ч.02 Власть Кроноса»
переходами, видео-эффектами, фейдом музыки и громкостями. Адаптация
enrich_oh_01.py под 19 сцен Ч.02:
  - переходы под драматургию свержения и пожирания детей (диссолв базово +
    зум-с-тряской на поворотных моментах + глитч-вспышка на ударе серпом
    и моменте пожирания + размытие-шар на смене эпох + бумажный шар на CTA);
  - halftone «Зеленые точки» на длительность аудио титула sentence_002;
  - crumpled paper SFX на каждом стыке «Бумажный шар» (только финальный 019→020);
...
Запуск (CapCut должен быть полностью закрыт, включая трей):
    python enrich_oh_02.py
    python enrich_oh_02.py --dry-run   # только показать план
"""
...
OH_02_DIR = DRAFTS / "От Хаоса до Олимпа Ч.02 Власть Кроноса"
OH_02_FILE = OH_02_DIR / "draft_content.json"
...
# Должна совпадать со scene_structure_oh_02.py: 19 сцен, у всех 1 шот.
# ─────────────────────────────────────────────────────────────────────

SCENE_LAYOUT: List[Tuple[str, int]] = [
    ("001", 1), ("003", 1), ("004", 1), ("005", 1), ("006", 1),
    ("007", 1), ("008", 1), ("009", 1), ("010", 1), ("011", 1),
    ("012", 1), ("013", 1), ("014", 1), ("015", 1), ("016", 1),
    ("017", 1), ("018", 1), ("019", 1), ("020", 1),
]
# Сумма: 19 ✓


# ─────────────────────────────────────────────────────────────────────
# План переходов под драматургию Ч.02:
#   001 → 003 хук+титул → угнетающее небо    — диссолв (плавный вход в основной текст)
#   003 → 004 страдание → решимость Геи      — Зум с тряской 2 (поворот: страдание → действие)
#   004 → 005 решимость → ковка серпа         — диссолв
#   005 → 006 серп на алтаре → Кронос берёт   — Резкий зум (важный выбор титана)
#   006 → 007 берёт серп → УДАР по небу       — Глитч-вспышка (момент свержения)
#   007 → 008 серебряная трещина → купол ушёл — Размытие (шар) (форма Урана меняется)
#   008 → 009 небо ушло → дети на свет        — диссолв (тёплый выдох после ужаса)
#   009 → 010 дети свободны → Афродита        — диссолв
#   010 → 011 Афродита → слёзы-дождь           — диссолв (поэтическая связка)
#   011 → 012 дождь Урана → трон Кроноса      — Размытие (шар) (смена эры: первобожества → титан-узурпатор)
#   012 → 013 свадьба → ПРОРОЧЕСТВО на полу   — Зум с тряской 2 (поворот: триумф → пророчество)
#   013 → 014 руны Геи → поглощение детей     — Глитч-вспышка (тёмный поворот сюжета)
#   014 → 015 пятеро котят → «одного за другим» — диссолв (короткая связка)
#   015 → 016 точка → Рея больше не могла     — диссолв
#   016 → 017 Рея решилась → бегство в горы   — Резкий зум (динамика бегства)
#   017 → 018 бегство → пещера-беременность   — диссолв (выдох-надежда)
#   018 → 019 беременность → клиффхэнгер Зевс — Зум с тряской 2 (мост к главному герою)
#   019 → 020 клиффхэнгер Зевс → CTA финал    — Бумажный шар (фактурный финальный + crumpled SFX)
#   (020 — последняя, переход не нужен)
# ─────────────────────────────────────────────────────────────────────

PLAN: List[Tuple[str, str, float, str]] = [
    # ⚠ ЗАПРЕЩЕНО: «Полутоновая вспышка» (7609529907026119941) и
    # «Пастельные блики» (7550260993348177213) — больше не используем
    # в проекте (решение 2026-05-18). См. memory feedback_banned_transitions_and_effects.
    ...
    ("007", "7159450506648097281", 1.20, "Размытие (шар)"),
    ...
    ("012", "7340177833508999681", 1.20, "Зум с тряской 2"),
    ("013", "7234817586234397186", 0.70, "Глитч-вспышка"),
    ("014", "6724845717472416269", 0.60, "叠化 (Dissolve)"),
    ("015", "6724845717472416269", 0.80, "叠化 (Dissolve)"),
    ("016", "7574908666210471221", 1.10, "Резкий зум"),
    ("017", "6724845717472416269", 0.80, "叠化 (Dissolve)"),
    ("018", "7340177833508999681", 1.20, "Зум с тряской 2"),
    ("019", "7249296835204878850", 1.30, "Бумажный шар"),
    # 020 — последняя сцена, переход не нужен
]
...
# на время произнесения титула (sentence_002). См. CAPCUT.md §5.1 +
# memory feedback_halftone_intro.
# ─────────────────────────────────────────────────────────────────────
...
# Halftone ложится на длительность аудио титула sentence_002.
HALFTONE_AUDIO_NAME = "sentence_002_v1.mp3"


# ─────────────────────────────────────────────────────────────────────
# Crumpled paper SFX — обязательный звук на каждом переходе «Бумажный шар».
# В ч.02 один «Бумажный шар» — на финальном стыке 019→020 (Зевс → CTA).
# См. CAPCUT.md §6.3 + memory feedback_paper_bag_sfx.
# ─────────────────────────────────────────────────────────────────────
...
def main() -> int:
    ...
    if not OH_02_FILE.is_file():
        print(f"Не нашёл драфт: {OH_02_FILE}")
        print("Сначала запусти: python build_oh_02.py")
        return 1
    ...
    print(f"Читаю Ч.02: {OH_02_FILE}")
    draft = json.load(open(OH_02_FILE, encoding="utf-8"))
    ...
    log_ht = apply_halftone_effect(draft)
    for line in log_ht:
        print(line)
    ...
    bkp = OH_02_FILE.with_suffix(".json.enrich-backup")
    shutil.copy2(OH_02_FILE, bkp)
    print(f"\nБэкап: {bkp.name}")

    json.dump(draft, open(OH_02_FILE, "w", encoding="utf-8"),
              ensure_ascii=False, separators=(",", ":"))
    for tgt_name in ("template-2.tmp", "draft_content.json.bak"):
        tgt = OH_02_DIR / tgt_name
        try:
            shutil.copy2(OH_02_FILE, tgt)
