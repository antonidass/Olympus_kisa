"""
Сборка CapCut-драфта «Тесей и Минотавр» через pyCapCut.

Что делает скрипт:
  1. Берёт готовые mp4 из `content/Тесей и Минотавр/video/`
     и mp3 из `content/Тесей и Минотавр/voiceover/audio/approved_sentences/`.
  ...
  4. Сохраняет драфт в папку CapCut. Потом открываешь CapCut, выбираешь
     проект «Тесей и Минотавр» и рендеришь финал оттуда.

Использование:
    python build_theseus.py                                  # автоопределение папки CapCut
    python build_theseus.py --drafts "D:\\...\\com.lveditor.draft"
    python build_theseus.py --name "Тесей v2"
    python build_theseus.py --dry-run                        # только напечатать план таймлайна
"""
...
from scene_structure_theseus import SCENES, Scene
...
DEFAULT_PROJECT_NAME = "Тесей и Минотавр"
...
THESEUS_ROOT = PROJECT_ROOT / "content" / "Тесей и Минотавр"

SCENES_DIR = THESEUS_ROOT / "video"
AUDIO_DIR = THESEUS_ROOT / "voiceover" / "audio" / "approved_sentences"
...
def build_draft(drafts_folder: Path, project_name: str) -> Path:
    ...
    # их всё равно перекрывает караоке из karaoke_theseus.py, иначе в
...
def main() -> int:
    p = argparse.ArgumentParser(description="Собрать CapCut-драфт «Тесей и Минотавр» через pyCapCut.")
    ...
    if drafts is None or not drafts.is_dir():
        print(
            ...
            "  python build_theseus.py --drafts "
