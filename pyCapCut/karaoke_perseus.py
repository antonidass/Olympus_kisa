"""
Караоке-субтитры для «Персей и Медуза».
...
Что делает:
  ...
  4. Удаляет старые сценные субтитры, кроме интро-оверлея
     «ПЕРСЕЙ И МЕДУЗА / МИФ ЗА МИНУТУ».
  5. Добавляет новые текстовые сегменты — по одному на слово, КАПСОМ.
  6. Кэширует транскрипции в pyCapCut/_karaoke_cache_perseus.json.

Требования:
  - CapCut должен быть закрыт.
  - Запускать из CosyVoice venv (там whisper + CUDA):
        external\\CosyVoice\\.venv_cosyvoice\\Scripts\\python.exe pyCapCut\\karaoke_perseus.py
...
CACHE_FILE = Path(__file__).resolve().parent / "_karaoke_cache_perseus.json"
...
# Строка 1: "ПЕРСЕЙ И МЕДУЗА" (белый), строка 2: "МИФ ЗА МИНУТУ" (бордовый #cb3a3a).
...
def transcribe_all(sentences: list[dict], model_name: str, allow_whisper: bool) -> None:
    ...
    try:
        ...
        raise SystemExit(
            ...
            r"  external\CosyVoice\.venv_cosyvoice\Scripts\python.exe pyCapCut\karaoke_perseus.py"
        ) from e
...
def build_intro_steps(intro_sentence: dict) -> list[tuple[int, int, str]]:
    """
    Один двухстрочный интро-титр на всю длительность sentence_003.
    В озвучке есть уточнение «Горгона», но в титре оставляем короткий
...
def build_intro_content_json(text: str, size: int,
    ...
    if nl_pos < 0:
        ...
        styles = [
            style([0, nl_pos], white),                  # "ПЕРСЕЙ И МЕДУЗА"
...
def main() -> int:
    ...
    # --- Диапазон интро (scene_02 содержит два аудио: hook-continuation + sentence_003 intro)
    intro_sentence = next(
        (s for s in sentences if re.match(r"sentence_003", s["fname"])),
        None,
    )
    if intro_sentence:
    ...
    print("Открой CapCut -> проект «Персей и Медуза» -> проверь.")
    return 0
