"""
Сборка CapCut-драфта «Мидас и золотое прикосновение» через pyCapCut.

Что делает скрипт:
  1. Берёт готовые mp4 из `content/Мидас и золотое прикосновение/video/`
     и mp3 из `content/Мидас и золотое прикосновение/voiceover/audio/approved_sentences/`.
  ...
  4. Сохраняет драфт в папку CapCut. Потом открываешь CapCut, выбираешь
     проект «Мидас и золотое прикосновение» и рендеришь финал оттуда.

Использование:
    python build_midas.py                                  # автоопределение папки CapCut
    python build_midas.py --drafts "D:\\...\\com.lveditor.draft"
    python build_midas.py --name "Мидас v2"
    python build_midas.py --dry-run                        # только напечатать план таймлайна
"""
...
from scene_structure_midas import SCENES, Scene
...
DEFAULT_PROJECT_NAME = "Мидас и золотое прикосновение"
...
MIDAS_ROOT = PROJECT_ROOT / "content" / "Мидас и золотое прикосновение"

SCENES_DIR = MIDAS_ROOT / "video"
AUDIO_DIR = MIDAS_ROOT / "voiceover" / "audio" / "approved_sentences"
...
def build_draft(drafts_folder: Path, project_name: str) -> Path:
    ...
    # их всё равно перекрывает караоке из karaoke_midas.py, иначе в
...
def main() -> int:
    p = argparse.ArgumentParser(description="Собрать CapCut-драфт «Мидас» через pyCapCut.")
