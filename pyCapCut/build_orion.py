"""
Сборка CapCut-драфта «Орион и Артемида» через pyCapCut.

Скопировано с build_orpheus.py с подменой путей и структуры сцен:
25 предложений озвучки → 23 видеошота в 23 таймлайн-сценах. CTA-аутро
у мифа нет — финальный кадр всегда заканчивается ровно по последней озвучке.

Использование:
    python build_orion.py                                  # автоопределение папки CapCut
    python build_orion.py --drafts "D:\\...\\com.lveditor.draft"
    python build_orion.py --name "Орион v2"
    python build_orion.py --dry-run                        # только напечатать план таймлайна
"""
...
from scene_structure_orion import SCENES, Scene
...
DEFAULT_PROJECT_NAME = "Орион и Артемида"
...
MYTH_ROOT = PROJECT_ROOT / "content" / "Орион и Артемида"
...
def build_draft(drafts_folder: Path, project_name: str) -> Path:
    ...
    # Остальные SCENE_TEXTS не добавляем — их перекроет karaoke_orion.py.
...
def main() -> int:
    p = argparse.ArgumentParser(description="Собрать CapCut-драфт «Орион и Артемида» через pyCapCut.")
