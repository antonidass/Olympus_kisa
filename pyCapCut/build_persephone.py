"""
Сборка CapCut-драфта «Персефона и Аид» через pyCapCut.

Скопировано с build_orpheus.py с подменой путей и структуры сцен:
24 предложения озвучки → 28 видеошотов в 24 сценах. CTA-аутро у мифа
нет — финальный кадр сидит на последнем предложении с trailing_pad.

Использование:
    python build_persephone.py                                  # автоопределение папки CapCut
    python build_persephone.py --drafts "D:\\...\\com.lveditor.draft"
    python build_persephone.py --name "Персефона v2"
    python build_persephone.py --dry-run                        # только напечатать план таймлайна
"""
...
from scene_structure_persephone import SCENES, Scene
...
DEFAULT_PROJECT_NAME = "Персефона и Аид"

VOICE_VOLUME = 1.0
ORIGINAL_CLIP_VOLUME = 0.34   # эталон Тесея
MUSIC_VOLUME = 0.1954         # эталон Тесея
...
MYTH_ROOT = PROJECT_ROOT / "content" / "Персефона и Аид"
...
def build_draft(drafts_folder: Path, project_name: str) -> Path:
    ...
    # Остальные SCENE_TEXTS не добавляем — их перекроет karaoke_persephone.py.
...
def main() -> int:
    p = argparse.ArgumentParser(description="Собрать CapCut-драфт «Персефона и Аид» через pyCapCut.")
    ...
    if drafts is None or not drafts.is_dir():
        print(
            ...
            "  python build_persephone.py --drafts "
