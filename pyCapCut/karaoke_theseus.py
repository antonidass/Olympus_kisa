"""
Караоке-субтитры для «Тесей и Минотавр».
...
Что делает:
  ...
  4. Удаляет старые сценные субтитры (16 шт.), кроме интро-оверлея
     «ТЕСЕЙ И МИНОТАВР / МИФ ЗА МИНУТУ».
  5. Добавляет новые текстовые сегменты — по одному на слово, КАПСОМ.
  6. Кэширует транскрипции в pyCapCut/_karaoke_cache_theseus.json.

Требования:
  - CapCut должен быть закрыт.
  - Запускать из CosyVoice venv (там whisper + CUDA):
        external\\CosyVoice\\.venv_cosyvoice\\Scripts\\python.exe pyCapCut\\karaoke_theseus.py
...
CACHE_FILE = Path(__file__).resolve().parent / "_karaoke_cache_theseus.json"
...
# Строка 1: "ТЕСЕЙ И МИНОТАВР" (белый), строка 2: "МИФ ЗА МИНУТУ" (красный #F20000)
...
def transcribe_all(sentences: list[dict], model_name: str, allow_whisper: bool) -> None:
    ...
    try:
        ...
        raise SystemExit(
            ...
            r"  external\CosyVoice\.venv_cosyvoice\Scripts\python.exe pyCapCut\karaoke_theseus.py"
        ) from e
...
def build_intro_steps(intro_sentence: dict) -> list[tuple[int, int, str]]:
    """
    Накапливающиеся шаги интро из 6 whisper-слов sentence_001
    (Тесей и Минотавр. Миф за минуту.):
      шаг 1: «ТЕСЕЙ»                                       [t0..t2]
      шаг 2: «ТЕСЕЙ И МИНОТАВР»                            [t2..t3]
      шаг 3: «ТЕСЕЙ И МИНОТАВР\\nМИФ»                      [t3..t4]
      шаг 4: «ТЕСЕЙ И МИНОТАВР\\nМИФ ЗА МИНУТУ»            [t4..end]
    ...
    # Интро в верхнем регистре
    u = [w.upper() for w in ws]
    # u = [ТЕСЕЙ, И, МИНОТАВР, МИФ, ЗА, МИНУТУ]
    steps_text = [
        u[0],                                                  # "ТЕСЕЙ"
        f"{u[0]} {u[1]} {u[2]}",                               # "ТЕСЕЙ И МИНОТАВР"
        f"{u[0]} {u[1]} {u[2]}\n{u[3]}",                       # "ТЕСЕЙ И МИНОТАВР\nМИФ"
        f"{u[0]} {u[1]} {u[2]}\n{u[3]} {u[4]} {u[5]}",         # "ТЕСЕЙ И МИНОТАВР\nМИФ ЗА МИНУТУ"
    ]
    # Тайминги: переход между шагами на старте слова, добавляющего новый блок
    steps_starts = [ts[0], ts[2], ts[3], ts[4]]
    steps_ends = [ts[2], ts[3], ts[4], end_us]
    out = []
...
def build_intro_content_json(text: str, size: int,
    ...
    if nl_pos < 0:
        ...
        styles = [
            style([0, nl_pos], white),              # "ТЕСЕЙ И МИНОТАВР"
