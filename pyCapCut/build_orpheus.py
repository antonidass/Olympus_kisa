"""
Сборка CapCut-драфта «Орфей и Эвридика» через pyCapCut.

Что делает скрипт:
  1. Берёт готовые mp4 из `content/Орфей и Эвридика/video/`
     и mp3 из `content/Орфей и Эвридика/voiceover/audio/approved_sentences/`.
  ...
  4. Сохраняет драфт в папку CapCut. Потом открываешь CapCut, выбираешь
     проект «Орфей и Эвридика» и рендеришь финал оттуда.

Использование:
    python build_orpheus.py                                  # автоопределение папки CapCut
    python build_orpheus.py --drafts "D:\\...\\com.lveditor.draft"
    python build_orpheus.py --name "Орфей v2"
    python build_orpheus.py --dry-run                        # только напечатать план таймлайна
"""
...
from scene_structure_orpheus import SCENES, Scene
...
DEFAULT_PROJECT_NAME = "Орфей и Эвридика"
...
ORPHEUS_ROOT = PROJECT_ROOT / "content" / "Орфей и Эвридика"

SCENES_DIR = ORPHEUS_ROOT / "video"
AUDIO_DIR = ORPHEUS_ROOT / "voiceover" / "audio" / "approved_sentences"
...
def build_draft(drafts_folder: Path, project_name: str) -> Path:
    ...
    # ── Субтитры ──
    # Только интро-сегмент сцены 002. Сценные SCENE_TEXTS не добавляем —
    # их всё равно перекрывает караоке из karaoke_orpheus.py, иначе в
...
def main() -> int:
    p = argparse.ArgumentParser(description="Собрать CapCut-драфт «Орфей и Эвридика» через pyCapCut.")
    ...
    if drafts is None or not drafts.is_dir():
        print(
            ...
            "  python build_orpheus.py --drafts "
