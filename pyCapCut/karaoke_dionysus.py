"""
Караоке-субтитры для «Дионис и Ариадна».
...
Что делает:
  ...
  4. Удаляет старые сценные субтитры, кроме интро-оверлея
     «ДИОНИС И АРИАДНА / МИФ ЗА МИНУТУ».
  5. Добавляет новые текстовые сегменты — по одному на слово, КАПСОМ.
  6. Кэширует транскрипции в pyCapCut/_karaoke_cache_dionysus.json.

Требования:
  - CapCut должен быть закрыт.
  - Запускать из CosyVoice venv (там whisper + CUDA):
    external\\CosyVoice\\.venv_cosyvoice\\Scripts\\python.exe pyCapCut\\karaoke_dionysus.py
...
CACHE_FILE = Path(__file__).resolve().parent / "_karaoke_cache_dionysus.json"
...
def transcribe_all(sentences: list[dict], model_name: str, allow_whisper: bool) -> None:
    ...
    try:
        ...
        raise SystemExit(
            ...
            r"  external\CosyVoice\.venv_cosyvoice\Scripts\python.exe pyCapCut\karaoke_dionysus.py"
        ) from e
...
def main() -> int:
    ...
    print("Открой CapCut -> проект «Дионис и Ариадна» -> проверь.")
    return 0
