"""
Сборка CapCut-драфта «Одиссей и Пенелопа» через pyCapCut.

Скопировано с build_persephone.py с подменой путей и структуры сцен:
25 предложений озвучки → 24 видеошота в 24 сценах. Хук (sent_001) +
интро-титул (sent_002) живут в одной сцене 001 на одном клипе scene_01.
CTA-аутро у мифа нет — финал сидит на sent_025 с trailing_pad.

Использование:
    python build_odysseus.py
    python build_odysseus.py --drafts "D:\\...\\com.lveditor.draft"
    python build_odysseus.py --name "Одиссей и Пенелопа v2"
    python build_odysseus.py --dry-run
"""
...
from scene_structure_odysseus import SCENES, Scene
...
US = 1_000_000
# В Одиссее зазор между аудио внутри сцены НОЛЬ: хук (sent_001) и интро-титул
# (sent_002) живут на одном 6-секундном клипе scene_01_v1.mp4 — любой gap
# толкает суммарную аудио-длину за 6.000s и pycapcut роняет VideoSegment.
GAP_US = 0
...
DEFAULT_PROJECT_NAME = "Одиссей и Пенелопа"

VOICE_VOLUME = 1.0
ORIGINAL_CLIP_VOLUME = 0.34   # эталон Персефоны
MUSIC_VOLUME = 0.1348         # эталон Персефоны
WHOOSH_VOLUME = 1.0
...
MYTH_ROOT = PROJECT_ROOT / "content" / "Одиссей и Пенелопа"
...
def mp4_duration_us(path: Path) -> int:
    """Длительность видеоклипа в µs. Кэшируется по пути."""
    sp = str(path)
    if sp in _mp4_dur_cache:
        return _mp4_dur_cache[sp]
    try:
    ...
    mi = MediaInfo.parse(sp)
    for t in mi.tracks:
        if t.track_type == "Video" and t.duration is not None:
            dur = int(float(t.duration) * 1000)
            _mp4_dur_cache[sp] = dur
            return dur
    raise RuntimeError(f"Не нашёл video-дорожку в {path}")


# ─────────────────────────────────────────────────────────────────────
# Автоопределение пути к CapCut Drafts на Windows
# ─────────────────────────────────────────────────────────────────────
...
def print_plan(plan: List[TimelineScene]) -> None:
    print(f"{'sid':<6} {'start':>7} {'dur':>7} {'audio':>7} {'trans':<20} text")
    print("-" * 80)
    for ts in plan:
...
def build_draft(drafts_folder: Path, project_name: str) -> Path:
    ...
    for ts in plan:
        ...
        for i, shot in enumerate(ts.scene.videos):
            ...
            clip_dur_us = mp4_duration_us(SCENES_DIR / shot.file)
            start_from_us = int(shot.start_from * US)
            available_us = clip_dur_us - start_from_us
            # Если клип короче нужного — берём весь клип, target оставляем
            # на shot_dur_us: pycapcut посчитает speed<1.0 (замедление,
            # тяги-видео покрытие). Если клип длиннее — режем по shot_dur.
            source_dur_us = min(shot_dur_us, available_us)
            kwargs = {
                "source_timerange": trange(start_from_us, source_dur_us),
            }
            if source_dur_us < shot_dur_us:
                print(f"  ⚠ клип {shot.file} ({available_us/US:.2f}s) короче нужных "
                      f"{shot_dur_us/US:.2f}s — будет замедление ×{source_dur_us/shot_dur_us:.2f}")
    ...
    for idx, ts in enumerate(plan):
        ...
        try:
            ...
            print(f"  → {ts.scene.sid:<6} {ts.scene.transition_after:<20} {dur_us/US:.2f}s")
    ...
    # ── Субтитры ──
    # Только интро-сегмент: двухстрочный титул накладывается на ту часть сцены 001,
    # где звучит sentence_002 (интро-аудио). karaoke_odysseus.py перекроет его
    # пословным караоке + накапливающимся титулом.
    intro_ts = next((ts for ts in plan if ts.scene.sid == "001"), None)
    if intro_ts is not None and len(intro_ts.audio_durs_us) >= 2:
        intro_offset_us = intro_ts.audio_durs_us[0] + GAP_US
        intro_dur_us = intro_ts.audio_durs_us[1]
        tseg = cc.TextSegment(
            "Одиссей и Пенелопа\nМиф за минуту",
            trange(intro_ts.start_us + intro_offset_us, intro_dur_us),
            style=TextStyle(
                size=14.0,
                color=(1.0, 1.0, 1.0),
                align=1,
...
def main() -> int:
    p = argparse.ArgumentParser(description="Собрать CapCut-драфт «Одиссей и Пенелопа» через pyCapCut.")
    ...
    if drafts is None or not drafts.is_dir():
        print(
            ...
            "  python build_odysseus.py --drafts "
