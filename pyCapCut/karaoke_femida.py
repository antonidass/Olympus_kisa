...
Что делает:
  ...
  6. Кэширует транскрипции в pyCapCut/_karaoke_cache_femida.json.

Требования:
  - CapCut должен быть закрыт.
  - Запускать из CosyVoice venv (там whisper + CUDA):
    external\\CosyVoice\\.venv_cosyvoice\\Scripts\\python.exe pyCapCut\\karaoke_femida.py
...
CACHE_FILE = Path(__file__).resolve().parent / "_karaoke_cache_femida.json"
...
# Строка 1: "ФЕМИ́ДА" (белый), строка 2: "МИФ ЗА МИНУТУ" (бордовый #cb3a3a).
...
def transcribe_all(sentences: list[dict], model_name: str, allow_whisper: bool) -> None:
    ...
    try:
        ...
        raise SystemExit(
            ...
            r"  external\CosyVoice\.venv_cosyvoice\Scripts\python.exe pyCapCut\karaoke_femida.py"
        ) from e
...
def main() -> int:
    ...
    print("Открой CapCut -> проект «Фемида» -> проверь.")
    return 0
