"""
Обогащает уже собранный CapCut-драфт «От Хаоса до Олимпа Ч.01 Хаос»
переходами, видео-эффектами, фейдом музыки и громкостями. Адаптация
enrich_persephone.py под 17 сцен Ч.01:
  - переходы под драматургию космогонии (плавный диссолв базово +
    фактурные переходы на ключевых стыках: рождение Геи, Тартар,
    Эрос, одиночество, рождение Урана, клиффхэнгер);
  - «Финальный круг» на CTA-сцене 019 (последние 1.5 с trailing_pad);
  - fade_out музыки 2.9 с в конце;
  - громкости как у Персефоны: voice 1.0, video 0.34, music 0.1348;
  - без 104% зума, без whoosh-SFX (у Ч.01 нет slide-переходов; whoosh
    добавим отдельно в enrich2-шаге, если понадобится).
...
Запуск (CapCut должен быть полностью закрыт, включая трей):
    python enrich_oh_01.py
    python enrich_oh_01.py --dry-run   # только показать план
"""
...
OH_01_DIR = DRAFTS / "От Хаоса до Олимпа Ч.01 Хаос"
OH_01_FILE = OH_01_DIR / "draft_content.json"
...
# Должна совпадать со scene_structure_oh_01.py: 17 сцен, у всех 1 шот.
# ─────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────
# План переходов под драматургию Ч.01:
#   001 → 003 хук+титул → рождение Геи  — Полутоновая вспышка (рифма halftone)
#   003 → 004 Гея появилась → опора     — диссолв
#   004 → 005 опора → Тартар тёмная     — Размытие шар (переход в тьму)
#   005 → 006 Тартар → Эрос             — диссолв
#   006 → 007 Эрос → без Эроса (контр)  — Пастельные блики (контраст света/тьмы)
#   007 → 008 контр → Эреб + Никта      — диссолв
#   008 → 009 двое → «Брат и сестра»    — диссолв (короткое)
#   009 → 010 близость → рождение света — Пастельные блики (свет из тьмы)
#   010 → 011 светлые дети → мир пуст   — диссолв
#   011 → 012 пуст → Гея одинока        — Размытие шар (погружение в одиночество)
#   012 → 013 одиночество → решимость   — Зум с тряской 2 (переломный момент)
#   013 → 014 решилась → рождение Урана — Резкий зум (рождение неба)
#   014 → 015 союз → 12 титанов         — диссолв
#   015 → 016 титаны → циклопы+гекатон. — диссолв
#   016 → 018 рождение → клиффхэнгер    — Глитч-вспышка (резкий поворот к угрозе)
#   018 → 019 клиффхэнгер → CTA         — Бумажный шар (фактурный финальный)
#   (019 — последняя, переход не нужен)
# ─────────────────────────────────────────────────────────────────────
...
PLAN: List[Tuple[str, str, float, str]] = [
    ...
    # в проекте (решение 2026-05-18). Заменены диссолвом.
    ...
    ("013", "7574908666210471221", 1.10, "Резкий зум"),
    ...
    ("018", "7249296835204878850", 1.30, "Бумажный шар"),
    # 019 — последняя сцена, переход не нужен
]
...
def main() -> int:
    ...
    if not OH_01_FILE.is_file():
        print(f"Не нашёл драфт: {OH_01_FILE}")
    ...
    print(f"Читаю Ч.01: {OH_01_FILE}")
    draft = json.load(open(OH_01_FILE, encoding="utf-8"))
    ...
    bkp = OH_01_FILE.with_suffix(".json.enrich-backup")
    shutil.copy2(OH_01_FILE, bkp)
    print(f"\nБэкап: {bkp.name}")

    json.dump(draft, open(OH_01_FILE, "w", encoding="utf-8"),
              ensure_ascii=False, separators=(",", ":"))
    for tgt_name in ("template-2.tmp", "draft_content.json.bak"):
        tgt = OH_01_DIR / tgt_name
        try:
            shutil.copy2(OH_01_FILE, tgt)
