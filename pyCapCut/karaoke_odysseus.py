"""
Караоке-субтитры для «Одиссей и Пенелопа».
...
Что делает:
  ...
  4. Удаляет старые сценные субтитры, кроме интро-оверлея
     «ОДИССЕЙ И ПЕНЕЛОПА / МИФ ЗА МИНУТУ».
  5. Добавляет новые текстовые сегменты — по одному на слово, КАПСОМ.
  6. Кэширует транскрипции в pyCapCut/_karaoke_cache_odysseus.json.

Требования:
  - CapCut должен быть закрыт.
  - Запускать из CosyVoice venv (там whisper + CUDA):
        external\\CosyVoice\\.venv_cosyvoice\\Scripts\\python.exe pyCapCut\\karaoke_odysseus.py
...
CACHE_FILE = Path(__file__).resolve().parent / "_karaoke_cache_odysseus.json"
...
# Строка 1: "ОДИССЕЙ И ПЕНЕЛОПА" (белый), строка 2: "МИФ ЗА МИНУТУ" (бордовый #cb3a3a).
INTRO_FONT_SIZE = 17.0                  # 2026-05-16: уменьшили с 22.75 — большой шрифт перекрывал halftone
...
def transcribe_all(sentences: list[dict], model_name: str, allow_whisper: bool) -> None:
    ...
    try:
        ...
        raise SystemExit(
            ...
            r"  external\CosyVoice\.venv_cosyvoice\Scripts\python.exe pyCapCut\karaoke_odysseus.py"
        ) from e
...
def build_intro_content_json(text: str, size: int,
    ...
    if nl_pos < 0:
        ...
        styles = [
            style([0, nl_pos], white),                  # "ОДИССЕЙ И ПЕНЕЛОПА"
