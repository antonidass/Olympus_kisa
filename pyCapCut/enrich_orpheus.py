"""
Обогащает уже собранный CapCut-драфт «Орфей и Эвридика» переходами,
...
Что делает:
  1. Читает draft_content.json «Орфей и Эвридика» (после build_orpheus.py).
  2. Подтягивает библиотеку из «Мидас и золотое прикосновение».
  3. По карте PLAN расставляет 27 переходов между 28 сценами Орфея,
     соблюдая контекст мифа.
  4. Ставит «Ожоги на пленке» на интро-сцену 002 и «Финальный круг»
     на финальное CTA-аутро 032.
  5. Делает fade_out 2.9s на фоновой музыке.
...
Запуск (CapCut должен быть полностью закрыт, включая трей):
    python enrich_orpheus.py
    python enrich_orpheus.py --dry-run   # только показать план
"""
...
THESEUS_DIR = DRAFTS / "Орфей и Эвридика"
...
# Должна совпадать со scene_structure_orpheus.py:
#   Орфей: 28 сцен с озвучкой, 31 mp4-шот + CTA-аутро 032
# ─────────────────────────────────────────────────────────────────────
...
SCENE_LAYOUT: List[Tuple[str, int]] = [
    ...
    ("016", 2), ("017", 1), ("018", 1), ("019", 1), ("020", 1),
    ("021", 1), ("022", 1), ("023", 1), ("024", 1), ("025", 1),
    ("026", 1), ("027", 1), ("028", 1), ("032", 1),
]
# Сумма: 32 ✓


# ─────────────────────────────────────────────────────────────────────
# План переходов под контекст мифа Орфея: от лирического начала к
# подземному спуску, затем к нарастающему запрету и трагическому разрыву.
# ─────────────────────────────────────────────────────────────────────

PLAN: List[Tuple[str, str, float, str]] = [
    ("001", "7340177833508999681", 1.20, "Зум с тряской 2"),
    ("002", "7550260993348177213", 1.20, "Пастельные блики"),
    ("003", "6724845717472416269", 0.80, "叠化 (Dissolve)"),
    ("004", "6724845717472416269", 0.80, "叠化 (Dissolve)"),
    ("005", "7550260993348177213", 1.10, "Пастельные блики"),
    ("006", "7327547930728993282", 0.60, "Поворот и изменение"),
    ("007", "7609529907026119941", 1.20, "Полутоновая вспышка"),
    ("008", "7159450506648097281", 1.20, "Размытие (шар)"),
    ("009", "7340177833508999681", 1.20, "Зум с тряской 2"),
    ("010", "7604808025253137682", 1.80, "Разделение рваной бумагой"),
    ("011", "7464433696658001213", 1.40, "Переход-зум"),
    ...
    ("016", "6724239584663704071", 1.20, "Свист"),
    ("017", "7620344224734629138", 1.20, "Растяжение влево"),
    ("018", "7620344224734629138", 1.20, "Растяжение влево"),
    ("019", "7620344224734629138", 1.20, "Растяжение влево"),
    ("020", "7620344224734629138", 1.20, "Растяжение влево"),
    ("021", "7340177833508999681", 1.20, "Зум с тряской 2"),
    ("022", "7262258307128103425", 1.20, "Зум с тряской"),
    ("023", "7604808025253137682", 1.80, "Разделение рваной бумагой"),
    ("024", "7159450506648097281", 1.30, "Размытие (шар)"),
    ("025", "6724845717472416269", 1.00, "叠化 (Dissolve)"),
    ("026", "7609529907026119941", 1.20, "Полутоновая вспышка"),
    ("027", "7350583934167552513", 1.20, "Рассеивание и зум"),
    ("028", "7249296835204878850", 1.066666, "Бумажный шар"),
    # 032 — финальная CTA-сцена, без перехода
]
...
WHOOSH_TRANSITION_EFFECT_IDS = {
    ...
    # Вправо бывает в живых драфтах, даже если его нет в текущей таблице.
    "6724227965965435396",
}
...
EFFECT_PLAN: List[Tuple[str, str, str]] = [
    ...
    ("032", "7613711779025358087", "Финальный круг"),
]
...
def make_crumpled_paper_segment(material_id: str, start_us: int, dur_us: int) -> dict:
    seg = make_whoosh_segment(material_id, start_us, dur_us)
    seg["volume"] = VOLUME_CRUMPLED_PAPER
    return seg
...
def apply_crumpled_paper_sfx(draft: dict) -> List[str]:
    ...
    first_idx = first_shot_index_per_sid()
    seg_idx = first_idx.get("032")
    if seg_idx is None:
        log.append("  ⚠ sid 032 не найден — paper-SFX пропускаю")
        return log

    main = next(t for t in draft["tracks"] if t["type"] == "video" and t.get("name") == "main")
    final_seg = main["segments"][seg_idx]
    final_start = int(final_seg["target_timerange"]["start"])
    full_dur = mp3_duration_us(CRUMPLED_PAPER_FILE)
    use_dur = min(CRUMPLED_PAPER_LEN_US, full_dur)
    start_us = max(0, final_start - CRUMPLED_PAPER_LEAD_US)
    ...
    sfx_track["segments"].append(make_crumpled_paper_segment(paper_mat["id"], start_us, use_dur))
    sfx_track["segments"].sort(key=lambda s: s["target_timerange"]["start"])
    log.append(
        f"  paper-SFX: Crumpled paper {start_us/1_000_000:.2f}s "
...
def main() -> int:
    ...
    if not THESEUS_FILE.is_file():
        print(f"Не нашёл драфт Орфея: {THESEUS_FILE}")
        print("Сначала запусти: python build_orpheus.py")
        return 1
    ...
    print(f"Читаю Орфея: {THESEUS_FILE}")
