"""
Обогащает уже собранный CapCut-драфт «Арахна» переходами,
видео-эффектами, фейдом музыки, громкостями и whoosh-SFX — точно так
же, как enrich_theseus.py, но с картой переходов под сюжет Арахны.
...
вручную) и применяем к Арахне. Это гарантирует:
  • все effect_id уже скачаны в CapCut Cache,
  • схема полей точно та, что CapCut ожидает.

Что делает:
  1. Читает draft_content.json «Арахна» (после build_arahna.py).
  2. Подтягивает библиотеку из «Мидас и золотое прикосновение».
  3. По карте PLAN расставляет 15 переходов между 16 сценами Арахны,
...
Запуск (CapCut должен быть полностью закрыт, включая трей):
    python enrich_arahna.py
    python enrich_arahna.py --dry-run   # только показать план
"""
...
ARAHNA_DIR = DRAFTS / "Арахна"
ARAHNA_FILE = ARAHNA_DIR / "draft_content.json"
MIDAS_FILE = DRAFTS / "Мидас и золотое прикосновение" / "draft_content.json"


# ─────────────────────────────────────────────────────────────────────
# Карта sid сцены → число шотов в треке.
# Должна совпадать со scene_structure_arahna.py:
#   Арахна: 16 сцен, 21 mp4-шот
# ─────────────────────────────────────────────────────────────────────

SCENE_LAYOUT: List[Tuple[str, int]] = [
    ("01", 1), ("02", 2), ("03", 1), ("04", 1), ("05", 1), ("06", 1),
    ("07", 1), ("08", 2), ("09", 1), ("10", 1), ("11", 2), ("12", 1),
    ("13", 1), ("14", 3), ("15", 1), ("16", 1),
]
# Сумма: 1+2+1+1+1+1+1+2+1+1+2+1+1+3+1+1 = 21 ✓


# ─────────────────────────────────────────────────────────────────────
# План переходов под контекст мифа Арахны.
#
# Логика подбора:
#   01→02  титульный standoff → мастерская                — Глитч-вспышка (бодрое начало)
#   02→03  мастерская → нимфы у гобелена                   — Пастельные блики (нежно, благоговение)
#   03→04  нимфы → портрет Арахны у станка                 — Поворот и изменение (тонкая смена)
#   04→05  портрет → дерзкая фраза перед толпой            — Глитч-вспышка (резкая дерзость)
#   05→06  дерзость → Олимп, Афина слышит                  — Растяжение влево (взлёт ввысь)
#   06→07  Олимп → старуха-Афина с предупреждением         — 叠化 (плавная маска богини)
#   07→08  старуха → Арахна смеётся                        — Зум с тряской 2 (резкая реакция)
#   08→09  смех → сброс маски + два станка готовы          — Полутоновая вспышка (божественный взрыв)
#   09→10  взрыв → две ткачихи параллельно                 — Размытие (шар) (фокус на действии)
#   10→11  ткачихи → гобелен Афины (торжество богов)       — Переход-зум (величие шедевра)
#   11→12  гобелен Афины → Арахна за работой + её гобелен  — Зум с тряской (бешеный темп)
#   12→13  гобелен Арахны → Афина в ужасе видит идеал      — Свист (быстрая смена ракурса)
#   13→14  ужас → гнев + касание + паучок + плетение       — Глитч-вспышка (магия превращения)
#   14→15  плетение в углу → паутина на рассвете           — 叠化 (мягкое утреннее растворение)
#   15→16  паутина → макро паутины (силуэт ткачихи в капле) — Рассеивание и зум (философский финал)
# ─────────────────────────────────────────────────────────────────────

PLAN: List[Tuple[str, str, float, str]] = [
    ("01", "7234817586234397186", 0.87, "Глитч-вспышка"),
    ("02", "7550260993348177213", 1.40, "Пастельные блики"),
    ("03", "7327547930728993282", 0.60, "Поворот и изменение"),
    ("04", "7234817586234397186", 0.60, "Глитч-вспышка"),
    ("05", "7620344224734629138", 1.40, "Растяжение влево"),
    ("06", "6724845717472416269", 1.07, "叠化 (Dissolve)"),
    ("07", "7340177833508999681", 1.00, "Зум с тряской 2"),
    ("08", "7609529907026119941", 1.27, "Полутоновая вспышка"),
    ("09", "7159450506648097281", 1.20, "Размытие (шар)"),
    ("10", "7464433696658001213", 1.40, "Переход-зум"),
    ("11", "7262258307128103425", 1.27, "Зум с тряской"),
    ("12", "6724239584663704071", 1.20, "Свист"),
    ("13", "7234817586234397186", 0.87, "Глитч-вспышка"),
    ("14", "6724845717472416269", 1.07, "叠化 (Dissolve)"),
    ("15", "7350583934167552513", 1.40, "Рассеивание и зум"),
...
def main() -> int:
    ...
    if not ARAHNA_FILE.is_file():
        print(f"Не нашёл драфт Арахны: {ARAHNA_FILE}")
        print("Сначала запусти: python build_arahna.py")
        return 1
    ...
    print(f"Читаю Арахну: {ARAHNA_FILE}")
    draft = json.load(open(ARAHNA_FILE, encoding="utf-8"))
    ...
    bkp = ARAHNA_FILE.with_suffix(".json.enrich-backup")
    shutil.copy2(ARAHNA_FILE, bkp)
    print(f"\nБэкап: {bkp.name}")

    json.dump(draft, open(ARAHNA_FILE, "w", encoding="utf-8"),
              ensure_ascii=False, separators=(",", ":"))
    for tgt_name in ("template-2.tmp", "draft_content.json.bak"):
        tgt = ARAHNA_DIR / tgt_name
        try:
            shutil.copy2(ARAHNA_FILE, tgt)
