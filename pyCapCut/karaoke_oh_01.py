"""
Караоке-субтитры для «От Хаоса до Олимпа — Ч.01 Хаос».

Адаптация karaoke_dionysus.py: пути контента ведут не в
`content/<PROJECT_NAME>/`, а в `content/От Хаоса до Олимпа/часть_01_Хаос/`
(сериал — поэтому иерархия глубже, чем у одиночных мифов). Имя
CapCut-драфта плоское: «От Хаоса до Олимпа Ч.01 Хаос». Интро-карточка
переписана под «ОТ ХАОСА ДО ОЛИМПА / ЧАСТЬ 1». Остальной пайплайн
идентичен Дионису.
...
Что делает:
  ...
  6. Кэширует транскрипции в pyCapCut/_karaoke_cache_oh_01.json.

Требования:
  - CapCut должен быть закрыт.
  - Запускать из CosyVoice venv (там whisper + CUDA):
    external\\CosyVoice\\.venv_cosyvoice\\Scripts\\python.exe pyCapCut\\karaoke_oh_01.py
...
def transcribe_all(sentences: list[dict], model_name: str, allow_whisper: bool) -> None:
    ...
    try:
        ...
        raise SystemExit(
            ...
            r"  external\CosyVoice\.venv_cosyvoice\Scripts\python.exe pyCapCut\karaoke_oh_01.py"
        ) from e
...
def main() -> int:
    ...
    print("Открой CapCut -> проект «От Хаоса до Олимпа Ч.01 Хаос» -> проверь.")
    return 0
