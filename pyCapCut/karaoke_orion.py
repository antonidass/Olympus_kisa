"""
Караоке-субтитры для «Орион и Артемида».
...
Что делает:
  ...
  4. Удаляет старые сценные субтитры, кроме интро-оверлея
     «ОРИОН И АРТЕМИДА / МИФ ЗА МИНУТУ».
  5. Добавляет новые текстовые сегменты — по одному на слово, КАПСОМ.
  6. Кэширует транскрипции в pyCapCut/_karaoke_cache_orion.json.

Требования:
  - CapCut должен быть закрыт.
  - Запускать из CosyVoice venv (там whisper + CUDA):
    external\\CosyVoice\\.venv_cosyvoice\\Scripts\\python.exe pyCapCut\\karaoke_orion.py
...
CACHE_FILE = Path(__file__).resolve().parent / "_karaoke_cache_orion.json"
...
# Строка 1: "ОРИОН И АРТЕМИДА" (белый), строка 2: "МИФ ЗА МИНУТУ" (бордовый #cb3a3a).
...
def transcribe_all(sentences: list[dict], model_name: str, allow_whisper: bool) -> None:
    ...
    try:
        ...
        raise SystemExit(
            ...
            r"  external\CosyVoice\.venv_cosyvoice\Scripts\python.exe pyCapCut\karaoke_orion.py"
        ) from e
...
def build_intro_content_json(text: str, size: int,
    ...
    if nl_pos < 0:
        ...
        styles = [
            style([0, nl_pos], white),                  # "ОРИОН И АРТЕМИДА"
