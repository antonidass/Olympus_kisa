"""
Карточка-объяснялка про Артемиду в драфт «Каллисто и Аркас».

Берёт за основу свиток из живого драфта «Персефона и Аид» (см. CAPCUT.md § 4.6,
эталон карточки-объяснялки) и клонирует его в Каллисто на момент, когда в
sentence_003 впервые звучит имя «Артемиды».

Тайминг: sentence_003 начинается на 7.58с (длина 4.43с). Карточка ставится
со смещением +0.5с от начала фразы — голос успевает произнести «Каллисто́ —
нимфа из свиты…», и ровно на «Артемиды» зритель видит свиток. Длительность
карточки — каноническая 3.78с (см. § 4.6).
"""
...
TARGET_PROJECT = "Каллисто и Аркас"
...
SCROLL_TRACK = "artemis_scroll"
LOGO_TRACK = "artemis_scroll_logo"
TEXT_TRACK = "artemis_scroll_text"
SFX_TRACK = "artemis_scroll_sfx"

# sentence_003 в Каллисто стартует на 7_583_333 µs, длина 4_433_333 µs.
# Сдвиг +500_000 µs внутрь фразы — карточка проявляется на «Артемиды».
SCENE_START_US = 8_083_333
SCENE_DURATION_US = 3_783_334
SCENE_END_US = SCENE_START_US + SCENE_DURATION_US

SOURCE_START_US = 15_150_000  # старт исходной карточки в Персефоне (см. add_graiae_scroll.py)
...
CALLOUT_TEXT = "АРТЕМИДА — БОГИНЯ ОХОТЫ\nИ ЛУНЫ, ДАЛА ОБЕТ\nВЕЧНОГО ЦЕЛОМУДРИЯ"
CALLOUT_NAME_LEN = len("АРТЕМИДА")  # сколько символов окрашено золотом
...
def main() -> int:
    ...
    if not TARGET_FILE.is_file():
        print(f"Не найден драфт Каллисто: {TARGET_FILE}")
        return 1
    ...
    backup = TARGET_FILE.with_suffix(".json.artemis-scroll-backup")
    ...
    print(f"Готово: добавлена карточка-объяснялка Артемида на {SCENE_START_US / 1_000_000:.3f}-{SCENE_END_US / 1_000_000:.3f}s")
