"""
Сборка CapCut-драфта «От Хаоса до Олимпа — Ч.01 Хаос» через pyCapCut.

Скопировано с build_persephone.py с подменой путей и структуры сцен:
19 предложений озвучки → 17 видеошотов. Сцены 001 и 016 совмещают
по два sent на один mp4 (хук+титул и циклопы+гекатонхейры
соответственно). CTA-сцена 019 держит trailing_pad=1.5 — под эффект
«Финальный круг» в enrich-шаге.

Использование:
    python build_oh_01.py                                  # автоопределение папки CapCut
    python build_oh_01.py --drafts "D:\\...\\com.lveditor.draft"
    python build_oh_01.py --name "От Хаоса до Олимпа Ч.01 v2"
    python build_oh_01.py --dry-run                        # только напечатать план таймлайна
"""
...
from scene_structure_oh_01 import SCENES, Scene
...
DEFAULT_PROJECT_NAME = "От Хаоса до Олимпа Ч.01 Хаос"
...
MYTH_ROOT = PROJECT_ROOT / "content" / "От Хаоса до Олимпа" / "часть_01_Хаос"
...
# CapCut онлайн-SFX, скачанные в кеш (как у Дионис и Ариадна).
# Pac — короткий «дзынь» для popup-стикеров.
# Mouse click — для UI-метафор (dashboard, alert).
...
# см. scene_structure_oh_01.py для маппинга sent ↔ scene_NN_v?.mp4.
STICKER_PLAN = [
    # video shot file, sticker image, x, y, scale, sfx
    ("scene_02_v1.mp4", "scene_03_gaia_legendary_primordial_achievement_unlocked.jpg", 0.0, 0.43, 0.265, "coin"),
    ("scene_04_v2.mp4", "scene_05_tartarus_one_star_review_warning.jpg", 0.54, 0.40, 0.245, "coin"),
    ("scene_05_v1.mp4", "scene_06_gravity_now_playing_eros_spotify.jpg", -0.54, 0.39, 0.245, "pac"),
    ("scene_10_v1.mp4", "scene_11_population_seven_activity_zero_dashboard.jpg", 0.54, 0.38, 0.245, "mouse"),
    ("scene_11_v1.mp4", "scene_12_loneliness_gauge_maxed_out.jpg", -0.54, 0.39, 0.245, "mouse"),
    ("scene_12_v1.mp4", "scene_13_birth_own_husband_rpg_quest_popup.jpg", 0.54, 0.40, 0.245, "pac"),
    ("scene_13_v1.mp4", "scene_14_husband_built_one_day_achievement.jpg", -0.54, 0.39, 0.245, "coin"),
    ("scene_14_v1.mp4", "scene_15_twelve_titans_legendary_edition_collection.jpg", 0.0, 0.43, 0.265, "coin"),
    ("scene_16_v1.mp4", "scene_18_revenge_quest_sharpening_sickle_tracker.jpg", -0.54, 0.39, 0.245, "pac"),
]
...
def build_draft(drafts_folder: Path, project_name: str) -> Path:
    ...
    # сцен (клиффхэнгер sent_018 = 7 сек, 12 титанов = 5 сек и т.п.) требует
    # большей длительности. pycapcut автоматически посчитает speed как
    # source.duration / target.duration, если передать оба timerange.
    # Используем cc.VideoMaterial для точного определения длительности (mutagen
    # может расходиться с pycapcut на 30-50 мс из-за разной интерпретации
    # mp4-контейнера).
    scene_last_shot = {}
    ...
    # Остальные SCENE_TEXTS не добавляем — их перекроет karaoke_oh_01.py.
...
def main() -> int:
    p = argparse.ArgumentParser(description="Собрать CapCut-драфт «От Хаоса до Олимпа Ч.01 Хаос» через pyCapCut.")
