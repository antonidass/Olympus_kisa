"""
Караоке-субтитры для «Баба-Яга».

Адаптация karaoke_persephone.py:
  - ИНТРО ОТКЛЮЧЕНО (intro_sentence=None принудительно) — у Бабы-Яги нет
    титульной строки «Баба-Яга. Миф за минуту.», sentence_002 — обычное
    сюжетное предложение. См. memory project_baba_yaga_no_title_line.
  - Все слова всех 23 предложений (включая sentence_001 = хук и sentence_002)
    получают обычное караоке снизу — без накопительного интро-титра поверху,
    без halftone, без typewriter-SFX интро.

Базовая логика karaoke_persephone.py:
...
Что делает:
  ...
  4. Удаляет старые сценные субтитры, кроме интро-оверлея
     «ПЕРСЕФОНА И АИД / МИФ ЗА МИНУТУ».
  5. Добавляет новые текстовые сегменты — по одному на слово, КАПСОМ.
  6. Кэширует транскрипции в pyCapCut/_karaoke_cache_baba_yaga.json.

Требования:
  - CapCut должен быть закрыт.
  - Запускать из CosyVoice venv (там whisper + CUDA):
        external\\CosyVoice\\.venv_cosyvoice\\Scripts\\python.exe pyCapCut\\karaoke_persephone.py
...
PROJECT_NAME = "Баба-Яга"
AUDIO_DIR = PROJECT_ROOT / "content" / PROJECT_NAME / "voiceover" / "audio" / "approved_sentences"
REF_TEXTS_DIR = PROJECT_ROOT / "content" / PROJECT_NAME / "voiceover" / "texts"
CACHE_FILE = Path(__file__).resolve().parent / "_karaoke_cache_baba_yaga.json"
WINDOWS_FONTS_DIR = Path(os.environ.get("LOCALAPPDATA", "")) / "Microsoft" / "Windows" / "Fonts"
...
TEXT_EFFECT_PATH = (
    ...
    / "effect"
    / "7298651529408498952"
    / "b32bee358ebfb58a17d9121ed2418ec2"
)
...
TYPEWRITER_ANIMATION = {
    ...
    "path": str(
        ...
        / "7255573948300005889" / "729d5e76cb22236d447b602d48d31b91"
    ).replace("\\", "/"),
    "platform": "all",
...
FOCUS_ANIMATION = {
    ...
    "path": str(
        Path(os.environ.get("LOCALAPPDATA", ""))
        / "CapCut" / "User Data" / "Cache" / "effect"
        / "7592158463770332432" / "5c93996262676ed0e150257ff4204fb4"
    ).replace("\\", "/"),
    "platform": "all",
    "resource_id": "7592158463770332432",
    "third_resource_id": "0",
    "source_platform": 1,
    "name": "Фокусировка",
...
DRAFT_DIR = LOCALAPPDATA / "CapCut" / "User Data" / "Projects" / "com.lveditor.draft" / PROJECT_NAME
...
# Караоке-слова — визуально ~14 в CapCut, raw JSON size = 10.
...
def make_text_animation(template: dict, duration_us: int | None = None) -> dict:
    anim = copy.deepcopy(template)
    if duration_us is not None:
        anim["duration"] = int(duration_us)
    return {
...
def make_text_effect() -> dict:
    return {
        "id": gen_id_dashed_upper(),
        "effect_id": TEXT_EFFECT_ID,
        "resource_id": TEXT_EFFECT_ID,
...
def make_keyboard_typing_audio() -> dict:
    return {
        "id": gen_id_dashed_upper(),
        "unique_id": "",
        "type": "sound",
        ...
        "source_platform": 0,
        "video_id": "",
        "effect_id": KEYBOARD_TYPING_EFFECT_ID,
        "resource_id": "",
        "third_resource_id": "",
        "category_id": "0",
        "intensifies_path": "",
...
def make_keyboard_typing_segment(material_id: str, start_us: int, duration_us: int) -> dict:
    duration_us = min(int(duration_us), KEYBOARD_TYPING_SOURCE_DURATION_US)
    return {
        "id": gen_id_dashed_upper(),
...
def add_keyboard_typing_sfx(draft: dict, start_us: int, duration_us: int) -> None:
    audio_ids = {
        m.get("id") for m in draft["materials"].setdefault("audios", [])
        if isinstance(m, dict) and m.get("name") == KEYBOARD_TYPING_NAME
    }
    for track in draft["tracks"]:
        if track.get("type") == "audio":
            track["segments"] = [
                seg for seg in track.get("segments", [])
                if seg.get("material_id") not in audio_ids
            ]

    audio = make_keyboard_typing_audio()
    draft["materials"]["audios"].append(audio)
    seg = make_keyboard_typing_segment(audio["id"], start_us, duration_us)

    track = next(
        (t for t in draft["tracks"] if t.get("type") == "audio" and t.get("name", "") == ""),
        None,
    )
    if track is None:
        track = {
...
def transcribe_all(sentences: list[dict], model_name: str, allow_whisper: bool) -> None:
    ...
    try:
        ...
        raise SystemExit(
            ...
            r"  external\CosyVoice\.venv_cosyvoice\Scripts\python.exe pyCapCut\karaoke_persephone.py"
        ) from e
...
def _apply_text_style_defaults(style: dict, font_path: Path, size: float, color: list[float], *, bold: bool) -> None:
    style["range"] = style.get("range", [0, 0])
    style["size"] = size
    style["font"] = _font_block(font_path)
    style["bold"] = bold
    style["effectStyle"] = _effect_style_block()
    fill = style.setdefault("fill", {})
    content = fill.setdefault("content", {"render_type": "solid"})
    content["render_type"] = "solid"
    content["solid"] = {"alpha": 1.0, "color": color}
...
def build_content_json(text: str, template_content: str,
    ...
    if styles:
        st = styles[0]
        st["range"] = [0, len(text)]
        _apply_text_style_defaults(st, ANTICVA_FONT, size, color, bold=True)
        parsed["styles"] = [st]
    else:
        style_base = {
            "fill": {"alpha": 1.0, "content": {"render_type": "solid", "solid": {"alpha": 1.0, "color": color}}},
...
def build_intro_steps(intro_sentence: dict) -> list[tuple[int, int, str]]:
    """
    Один двухстрочный интро-титр из 6 whisper-слов sentence_002
    (Баба-Яга. Миф за минуту.):
      «ПЕРСЕФОНА И АИД\\nМИФ ЗА МИНУТУ»                  [t0..end]
    Тайминги t0..t5 — whisper-начала слов, end = конец голосового сегмента.
    Пунктуация на концах слов срезается.
    """
    ...
    ws = [_strip_trailing_punct(w["word"]) for w in words[:6]]

    u = [w.upper() for w in ws]
    text = f"{u[0]} {u[1]} {u[2]}\n{u[3]} {u[4]} {u[5]}"
    return [(base_us, end_us - base_us, text)] if end_us > base_us else []
...
def build_intro_content_json(text: str, size: int,
    ...
    if nl_pos < 0:
        ...
        styles = [
            style([0, nl_pos], white),                  # "ПЕРСЕФОНА И АИД"
...
def make_intro_text_material(template_mat: dict, text: str) -> dict:
    ...
    font_info = _font_block(STROME_FONT)
    ...
    for k, v in {
        ...
        "text_color": "#cb3a3a", "source_from": "",
        "font_path": str(STROME_FONT).replace("\\", "/"),
        "font_name": "", "font_title": "none", "font_id": "", "font_resource_id": "",
        "bold_width": 0.0,
        "bubble": "", "has_shadow": False, "shadow_color": "",
...
def make_text_material(template_mat: dict, text: str,
    ...
    m["font_path"] = str(ANTICVA_FONT).replace("\\", "/")
    m["font_name"] = ""
    m["font_title"] = "none"
    m["font_id"] = ""
    m["font_resource_id"] = ""
    m["bold_width"] = 0.00800000037997961
    m["words"] = {"text": [text], "start_time": [0], "end_time": [0]}
    m["source_from"] = ""
    m["bubble"] = ""
    m["has_shadow"] = False
...
def main() -> int:
    ...
    # --- ИНТРО ОТКЛЮЧЕНО для этого мифа.
    # У Бабы-Яги нет титульной строки «Баба-Яга. Миф за минуту.» — sentence_002
    # это обычное сюжетное предложение «Её хозяйка — старуха с одной костяной
    # ногой». Поэтому накопительный двухстрочный титр на нём не строим, halftone
    # не кладём, typewriter-SFX не добавляем. См. memory
    # project_baba_yaga_no_title_line.
    intro_sentence = None
    ...
    for step_start_us, step_dur_us, step_text in intro_steps:
        ...
        intro_anim = make_text_animation(TYPEWRITER_ANIMATION, min(1_666_666, step_dur_us))
        intro_effect = make_text_effect()
        draft["materials"]["texts"].append(intro_mat)
        draft["materials"].setdefault("effects", []).append(intro_effect)
        draft["materials"].setdefault("material_animations", []).append(intro_anim)
        intro_seg["extra_material_refs"] = [intro_anim["id"], intro_effect["id"], intro_effect["id"]]
    ...
    for start_us, end_us, word in words:
        ...
        focus_anim = make_text_animation(FOCUS_ANIMATION, 100_000)
        draft["materials"]["texts"].append(mat)
        draft["materials"].setdefault("material_animations", []).append(focus_anim)
        seg["extra_material_refs"] = [focus_anim["id"]]
