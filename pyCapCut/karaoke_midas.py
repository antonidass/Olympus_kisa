"""
Караоке-субтитры для «Мидас и золотое прикосновение».
...
Что делает:
  ...
  2. По каждому sentence_NNN_vX.mp3 из approved_sentences запускает
     openai-whisper (CUDA, modul `medium` по умолчанию) и вытягивает
  ...
  4. Удаляет старые сценные субтитры (25 шт.), кроме интро-оверлея
     «Царь Мидас / Миф за минуту».
  5. Добавляет новые текстовые сегменты — по одному на слово.
  6. Кэширует транскрипции в pyCapCut/_karaoke_cache.json, чтобы
     повторный запуск был мгновенным (если поменяешь только укладку —
     удалять кэш не надо).

Требования:
  - CapCut должен быть закрыт.
  - Запускать из CosyVoice venv (там whisper + CUDA):
        external\\CosyVoice\\.venv_cosyvoice\\Scripts\\python.exe pyCapCut\\karaoke_midas.py
...
MIDAS_NAME = "Мидас и золотое прикосновение"
AUDIO_DIR = PROJECT_ROOT / "content" / MIDAS_NAME / "voiceover" / "audio" / "approved_sentences"
REF_TEXTS_DIR = PROJECT_ROOT / "content" / MIDAS_NAME / "voiceover" / "texts"
CACHE_FILE = Path(__file__).resolve().parent / "_karaoke_cache.json"

LOCALAPPDATA = Path(os.environ.get("LOCALAPPDATA", ""))
DRAFT_DIR = LOCALAPPDATA / "CapCut" / "User Data" / "Projects" / "com.lveditor.draft" / MIDAS_NAME
...
# Rubik-Bold — уже в ресурсах проекта, шаблон (template) материалов
# использует его. Клонируем, меняем только размер / цвет / позицию.

# ВНИМАНИЕ: в CapCut координата y для text-сегмента инвертирована —
# ПОЛОЖИТЕЛЬНЫЕ значения двигают ВНИЗ, отрицательные — ВВЕРХ.
# Оригинальный "нижний" субтитр в template имеет transform.y = +0.78.
...
# Интро: накапливающийся текст из 4 шагов.
# Строка 1: "Царь Мидас" (белый), строка 2: "Миф за минуту" (красный #F20000)
INTRO_FONT_SIZE = 36                    # как пользователь выставил вручную
...
def strip_stresses(text: str) -> str:
    ...
    """
    # Нормализуем в NFD, удаляем акут, собираем обратно в NFC.
...
def tokenize_reference(text: str) -> list[str]:
    ...
    # потом разбиваем каждый токен по обычным дефисам (но не по em-dash «—»)
    split = []
    for t in raw:
...
def align_whisper_to_reference(whisper_words: list[dict], ref_words: list[str]) -> list[dict]:
    ...
    # стоимость подстановки: 0 если ключи совпадают, иначе 1
    ...
    for kind, wi, ri in ops:
        if kind == "sub":
            ...
            # whisper услышал лишнее — пропускаем
            continue
        else:  # ins_ref — эталон есть, whisper пропустил. Вставляем, деля пополам соседа.
...
def substitute_with_reference(sentences: list[dict]) -> None:
    ...
    печатаем отдельно, чтобы было видно, где whisper ошибся по числу.
    """
    total_subst = 0
    matched = 0
    ...
    for s in sentences:
        ...
        if len(ref) == len(words):
            ...
            # Рассинхрон по числу слов — DP-выравнивание
...
def transcribe_all(sentences: list[dict], model_name: str, allow_whisper: bool) -> None:
    ...
    try:
        ...
        raise SystemExit(
            ...
            r"  external\CosyVoice\.venv_cosyvoice\Scripts\python.exe pyCapCut\karaoke_midas.py"
        ) from e
    ...
    for s in sentences:
        ...
        res = model.transcribe(
            ...
            fp16=False,     # стабильнее на маленьких моделях/CUDA без tensor cores
...
def build_content_json(text: str, template_content: str,
    ...
    if styles:
        ...
        if stroke:
            st["strokes"] = [stroke]
        parsed["styles"] = [st]  # оставляем только один стиль
    else:
        style_base = {
            "fill": {"alpha": 1.0, "content": {"render_type": "solid", "solid": {"alpha": 1.0, "color": color}}},
            "range": [0, len(text)],
...
def build_intro_steps(intro_sentence: dict) -> list[tuple[int, int, str]]:
    """
    Накапливающиеся шаги интро из 5 whisper-слов sentence_001:
      шаг 1: «Царь»                                      [t1..t2]
      шаг 2: «Царь Мидас»                                [t2..t3]
      шаг 3: «Царь Мидас\\nМиф»                          [t3..t4]
      шаг 4: «Царь Мидас\\nМиф за минуту»                [t4..end]
    Тайминги t1..t5 — whisper-начала слов, end = конец голосового сегмента.
    Пунктуация на концах слов срезается, чтобы получилось «Мидас», а не «Мидас.».
    """
    ...
    # Интро в верхнем регистре: «ЦАРЬ МИДАС / МИФ ЗА МИНУТУ»
    u = [w.upper() for w in ws]
    steps_text = [
        u[0],                                              # "ЦАРЬ"
        f"{u[0]} {u[1]}",                                  # "ЦАРЬ МИДАС"
        f"{u[0]} {u[1]}\n{u[2]}",                          # "ЦАРЬ МИДАС\nМИФ"
        f"{u[0]} {u[1]}\n{u[2]} {u[3]} {u[4]}",            # "ЦАРЬ МИДАС\nМИФ ЗА МИНУТУ"
    ]
...
def build_intro_content_json(text: str, size: int,
    ...
    if nl_pos < 0:
        ...
        styles = [
            style([0, nl_pos], white),              # "ЦАРЬ МИДАС"
...
def make_intro_text_material(template_mat: dict, text: str) -> dict:
    """Material с мульти-стилем (белое/красное по строкам), size 16."""
    ...
    # Сбрасываем AI-caption поля + меняем тип на "text" чтобы CapCut
    # не применял auto-layout субтитров и уважал transform.y.
    for k, v in {
        "type": "text",             # ← ключ: НЕ "subtitle" (auto-captions)
        "line_spacing": -0.20,      # плотнее между строками (−20%)
...
def make_text_material(template_mat: dict, text: str,
    ...
    m["base_content"] = m["content"]
    # Меняем тип на "text" (не "subtitle") — чтобы CapCut не применял
    # auto-layout AI-субтитров и уважал наш transform.y.
    m["type"] = "text"
    m["recognize_task_id"] = ""
    m["recognize_text"] = ""
    m["recognize_model"] = ""
...
def layout_words(sentences: list[dict]) -> list[tuple[int, int, str]]:
    ...
    for s in sentences:
        ...
        for i, w in enumerate(words):
            ...
            # страховка — не выходить за предложение
...
def main() -> int:
    ...
    #     вставки (он часто дробит их на безымянные треки).
    text_tracks_all = [t for t in draft["tracks"] if t.get("type") == "text"]
    # «наши» дорожки = karaoke + intro + безымянные (subtitles не трогаем)
    ...
    # --- Интро: накапливающийся текст, 4 шага
    #     Царь → Царь Мидас → Царь Мидас\nМиф → Царь Мидас\nМиф за минуту
    ...
    # --- синхронизируем CapCut-овские кэши. Без этого CapCut при открытии
    #     достаёт "last known state" из template-2.tmp и .bak, игнорируя
    #     наши изменения, а потом перезаписывает draft_content.json обратно.
    ...
    print("Открой CapCut → проект «Мидас и золотое прикосновение» → проверь.")
    return 0
